...
**Phase:** Planning, in its final stretch. 33 of 35 plan documents are complete and lint-clean: document 34 (work breakdown) and document 20 (traceability) are done. Left: 30 scorecard, 00 executive summary, and the v9.1 brief corrections.
...
1. **20** traceability matrix: **done**, generated by `node tools/plan-build/gen-20.mjs` (860 requirements, every one with a test; 333 derived acceptance tests in the 950 range).
