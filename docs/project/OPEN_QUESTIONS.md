...
Thirty-one questions are open. `docs/plan/01-questions-and-assumptions.md` restates each one with its likelihood, impact, score and RISK identifier in `docs/plan/18-risk-register.md`, and kit-lint rule R26 fails when the two files disagree on which questions are open.

Six questions from v8 were closed in v9 by decision records that are still **Proposed, awaiting product owner confirmation**. They appear at the bottom as defaults in force, not as settled answers, until the product owner accepts each record. The four value decisions of ADR-0019, which the product owner Accepted on 2026-09-22, are settled.
...
| 9 | Which regional plug-ins first? | Interfaces in Tier 1, implementations per country in phase 5 | Integration work and the partner story. E-invoicing is the pressing case: SL-FIN-444 needs a ZATCA or JoFotara plug-in when Finance ships in phase 3 (RISK-23, RISK-53) | Product owner | Open |
...
| 30 | Which count bills a tenant: students enrolled on the billing date, prorated by day (master brief Section 36, REQ-PLT-009), or any student enrolled for at least one day of the month (BR-FIN-017)? | Students enrolled on the billing date, prorated by day, as the brief and REQ-PLT-009 state. Until decided, BR-FIN-017, Finance's count job and SL-PLT-010 (which lists BR-FIN-017 beside REQ-PLT-009) are in conflict with this default | A school is invoiced on a figure other than the one its contract states, or a correction reissues invoices after launch. RISK-52 (16). On the decisions workshop agenda; due before SL-PLT-010 starts in phase 1 | Product owner | Open |
