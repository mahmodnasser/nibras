...
| `dev-smoke.yml` | One-command local start, one Podman configuration and one Docker Engine configuration, then the health endpoint of every service | yes | yes | none | Nightly and on `deploy/compose/**`, `docs/dev-setup/**`, `tools/dev-setup/**` | Proves the setup guides rather than remembering them; catches the Podman network naming edge case |
