...
| Apple build capacity | iOS builds per month × minutes per build × flavours | Hosted runner minutes × price, or one Mac host once | Hosted macOS minute 0.08; Mac build host 1,000 once | Fleet cost | 40 builds × 25 min × 0.08 = 80 per flavour | Open question 14; RISK-04; a fleet line, not per school; bounded at the shared app plus at most 5 white-label flavours a cycle, 480 a month (`09-mobile-structure.md` §5.3) |
...
| 2026-09-26 | Group F review, round 3 | Blocked: the group scored below 4 on Completeness, because document 31 listed no transition-test ids per workflow | None of the group's blocking gaps was in this document; `TC-PERF-800` is a manual review step (Testability, not blocking), and this record stopped at "awaiting the round 3 score" |
| 2026-09-26 | Group F review, round 4 | Blocked: the group scored below 4 on Consistency, because SL-ACA-207 in document 34 built `LaunchLtiTool` in phase 2 against the default of document 17 | None in this document; `TC-PERF-800` still a manual review step (Testability, not blocking); the prices remain illustrative (RISK-28) |
| 2026-09-26 | Round-4 scorecard, Group F, then remediation round 5 | Amended; awaiting the round 5 score | The verification row of `TC-PERF-800` states plainly that it is a named review step under ADR-0021, who runs it, what is compared and when, why it is not a script, and what would make it one; the round 3 and round 4 verdicts recorded above |
| 2026-09-26 | Round-5 scorecard, remediation round 6, `TC-PERF-800` run by script | Amended; awaiting the round 6 score | Group F was approved with minor gaps; for this document, `TC-PERF-800` was still a manual step. `tools/plan-build/capacity-28.mjs` now performs it with `.ps1` and `.sh` wrappers, and the verification row and the test case name it. Run on Linux: 160 figures checked, 0 mismatched, and the scale-tier off-peak vCPU of part 2.5 (printed 72.1, recomputed 72.6) reported as carried by open point 12; the year-one database total of part 2.8 reads as held rather than written, since the notification log keeps only its 90-day share. No figure changed |
| 2026-09-26 | Remediation round 6, document 09 alignment | Amended | The Apple build line states the bound document 09 plans: the shared app plus at most 5 white-label flavours a cycle, 480 USD a month |

## How this document is verified

| Claim | Proof | Where it runs |
|---|---|---|
| The arithmetic of parts 2.5, 2.8, 4.3, 4.4, 4.5 and 5 | `TC-PERF-800`, defined below: every figure recomputes from the unit prices of part 4.1, the formulas of part 4.2 and the tables of part 2, to the cent. It is performed by the script `tools/plan-build/capacity-28.mjs` (with `capacity-28.ps1` and `capacity-28.sh`), which reads the inputs from this document itself (the unit prices of part 4.1, the month and packing of part 1, the tables of parts 2.3, 2.4, 2.6 and 2.8, and the drivers in the Working and Formula cells of parts 4.3 to 5), recomputes 160 figures by the part 4.2 formulas and exits 1 on any printed figure that differs; the one difference this document carries, the scale-tier off-peak vCPU of part 2.5, is reported against open point 12 and fails the run if that open point is removed while the difference stays. The formulas are therefore written twice, in part 4.2 and in the script, and the script fails when the two disagree on any printed figure. `performance-reviewer` runs it at each Group F review and at every change to part 2, part 4 or part 5, and records the result in the review record (ADR-0021 proof kind 2, a named review step, now carried out by a script rather than by hand) | Group F review; again at every change to part 2, part 4 or part 5 |
