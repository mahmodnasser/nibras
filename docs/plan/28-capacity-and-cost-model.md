...
|---|---|---|
| The arithmetic of parts 2.5, 2.8, 4.3, 4.4, 4.5 and 5 | `TC-PERF-800`, defined below: every figure recomputes from the unit prices of part 4.1, the formulas of part 4.2 and the tables of part 2, to the cent. It is a named review step under ADR-0021 (proof kind 2), not a script, and says so plainly: `performance-reviewer` recomputes every figure and compares it with the printed value, at each Group F review and at every change to part 2, part 4 or part 5. It stays a review step because the figures are plan arithmetic over illustrative prices (RISK-28) that change only when those parts change, and a script would restate the part 4.2 formulas in a second place that must be kept in step with the prose; a recomputation script under `tools/plan-build` is the upgrade if those parts change more than once a phase. The round 3 run is in the review record | Group F review; again at every change to part 2, part 4 or part 5 |
