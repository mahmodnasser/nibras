...
Points 1 to 7 are the cycle classifications that Section 5.1 used to defer to the Group B review. Each now has an owner and a default in force, which is the classification Section 5.1 proposes; the architect records the two new cycle classes (orchestrators other than Requests, and the job reply channel) in the ADRs that `04-architecture-overview.md` Open points names.
...
| 8 | Does School consume `assessment.grades.locked.v1` (row 13)? | No binding: Saga 4 checks the lock with the `ConfirmYearResultsLocked` command, as `06-services/school.md` open point 12 records, which supersedes the recommendation in row 13 | Architect | Rollover runs on unlocked grades if the command is skipped; the fix is a binding and an Appendix E change under an ADR | 2 | 3 | 6 | none |
