...
**Group** C · **Requirement areas covered** PERF, with DATA where partitioning and pooling touch a query · **Last updated** 2026-09-26 by the platform plan
...
**Who evicts the backend-for-frontend entries.** Gateway, Bff.Web and Bff.Mobile consume no event (`11-messaging-architecture.md` part 1), so a routing key in the "Invalidated by" column above names the change whose publisher broadcasts the eviction, not a binding in the BFF. The handler that writes the event's outbox row also publishes the affected tag on `state:invalidate` after commit (Section 2.6), without touching the BFF keys, which its own ACL user cannot reach: Requests for `requests.task.assigned.v1`, Notification for `notification.notification.delivered.v1` and Attendance for `attendance.attendance.not-marked.v1` (tag `user` of the recipient), Identity for `identity.permissions.changed.v1` and `identity.role.changed.v1` (tag `user`), and Platform for `platform.feature-flag.changed.v1` and `platform.terminology.changed.v1` (tag `tenant`). Each BFF instance, subscribed to the channel, drops its matching L1 entries and removes the matching L2 entries under its own ACL user (`svc_bff_web`, `svc_bff_mobile`, Section 2.3). Without a broadcast an entry still lives only to its 60-second lifetime, which the budget tolerates.
...
| `svc_gateway` | `redis-cache`, `redis-state` | Read-only `~nibras:platform:platform:tenant-by-host:*`, `~nibras:platform:identity:jwks:*` (the key set of Section 1.1) and `~nibras:*:platform:*` (maintenance state and the security settings the edge reads, `06-services/gateway.md` §12); `~state:ratelimit:*:edge:*`; read-only `~state:revoked:*` (Section 2.2) | `&state:invalidate` (subscribe only; the Gateway's `InvalidationSubscriber` drops its L1 entries on it, `06-services/gateway.md` §10) | `+@read` on cache; `+@read +@write` on its rate-limit prefix; `+@read` only on `state:revoked:`, because Identity is the sole writer; `+subscribe` on the channel |
...
| The PgBouncer pools saturate at the peak, or transaction mode breaks under row-level security and prepared statements | 3 | 4 | The pool sizes and timeouts of Section 5 taken from load evidence rather than guessed, prepared statements off in the first release, the `PgBouncerPoolSaturated` alert, and `TC-PERF-025`, `TC-PERF-026` with `TC-DATA-643` to `TC-DATA-645` (document 10) running every integration suite through PgBouncer | Architect | RISK-14 |
| A budget in Section 7 is broken quietly: command counts and p95 drift upward one merge at a time until the 08:00 peak misses it | 3 | 3 | The exact command-count gate and the 10 percent p95 gate on `docs/perf/<service>/baseline.json`, the always-on slow-query interceptor, the `EXPLAIN (ANALYZE, BUFFERS)` evidence rule of Section 12 that blocks a service from being declared done, and the nightly k6 baseline comparison `TC-TST-219` | Performance engineer | none; score 9, local (the gates above are the control; no register risk covers budget drift) |
| Calendar-aware scaling and the pre-peak warm-up of Section 9 are tuned on demo data and do not match a real school calendar, so the peak arrives unwarmed | 3 | 3 | The warm-up job is idempotent and marked per tenant per day in `state:warmup:` (Section 2.2), skips L2 when Redis is down, and is driven by the time-zone bands and KEDA cron triggers of `15-deployment-and-operations.md` part 11 rather than by a fixed clock; N-11 measures the cold-cache case as the floor | Performance engineer | none; score 9, local. The peak consequence, a stale or cold entry at 08:00, is RISK-13's; this row is the calendar input to it |

### 14. Platform notes and signature features

| Concern | What holds here | Proof and runner |
|---|---|---|
| Runner | Query budgets, `EXPLAIN` captures, the cache-entry suite and the pooling tests run in each service's integration suite on the Linux runner (`ubuntu-latest`); the caching and persistence building blocks also run on `windows-latest`, the only Windows integration runs document 33 part 4 keeps (`BuildingBlocks`, `Documents`). The k6 scenarios of Section 8 run on the Linux load and scale tiers only, because the budgets are measured where production runs. `dev-smoke` runs on `ubuntu-latest`, `windows-latest` and `macos-latest` and measures no budget | Document 33 part 4; `TC-TST-219` on the load tier |
| Culture-sensitive values | Every cache key, `redis-state` key and metric label is machine-facing: numbers and dates in it are formatted under the invariant culture, so an `ar-SA` process, whose default calendar is Um Al Qura, never writes a Hijri date into `state:warmup:{tenant}:{date}` or a period key | `TC-PERF-750`, on `ubuntu-latest` and `windows-latest` through the caching building block |
| Time zones | Warm-up, scale-up and the integrity jobs run on each tenant's own time zone from the band table of `15-deployment-and-operations.md` part 11, never on the host clock, which is UTC in every image | `TC-PERF-751` on the Linux runner; `TC-PLAT-005` (document 33) for the pinned tzdata |
| Right-to-left and Arabic payloads | The payload budgets of Section 10 are measured on Arabic examples, which are about twice the bytes of English ones in UTF-8; the Spectral rule `nibras-example-arabic` of document 22 guarantees such an example exists for every schema with bilingual text or a person's name | The payload-size contract tests, `TC-BFF-011` and `TC-BFF-754` (Bff.Web sheet) |
| Devices without Google services | They receive no push while the app is closed, so they catch up by one delta pull when opened (REQ-NOT-019, REQ-MOB-038); that pull is part of the N-08 sync storm and costs the same budget as any other device's | `TC-NOT-610` (Notification sheet); N-08 on the load tier |

**Signature features.** This document owns none; its budgets are what features 1 (Today dashboards, N-05), 3 (sixty-second attendance, N-01), 6 (Report Card Studio, N-02), 32 (emergency mode, N-09), 34 (teacher five-minute mode) and 44 (low-bandwidth mode) need to be shown in under a minute, cited by their Appendix W numbers. Feature 39, calendar-aware scaling, is no longer a signature feature (ADR-0019): Section 9 builds it as an engineering capability and N-01 and N-11 measure it. Each feature's moment, rung, autonomy, the requirements and slices that build it, its Appendix O step and its demo test are held once, in the "Signature feature trace" table of `32-product-differentiation-and-demo.md`, and are not copied here.
...
| Keys, markers and warm-up times do not depend on the host culture or clock | `TC-PERF-750` on `ubuntu-latest` and `windows-latest`, `TC-PERF-751` on `ubuntu-latest` (Section 14) | Every pull request touching the caching building block or the warm-up job |
...
This document defines the partition-detach and pooling tests below, whose setup and pass conditions are spelled out in the detach table of Section 4 and the pooled-connection table of Section 5; `TC-DATA-641` and the pooled-connection tests `TC-DATA-643` to `TC-DATA-645` are cited from their owner, document 10. It also defines `TC-PERF-750` and `TC-PERF-751`, the culture and time-zone tests of the Platform notes in Section 14.
...
| TC-PERF-750 | Given the caching building block's key builder run with the process culture set to `ar-SA`, then `en-US`, then the invariant culture, when it builds a warm-up marker for 2026-09-27, a timetable-day key and a usage-period key, then the three runs produce byte-identical keys with a Gregorian ISO 8601 date and Latin digits, on `ubuntu-latest` and `windows-latest` | REQ-PLAT-019, REQ-PERF-020 |
| TC-PERF-751 | Given tenants in `Asia/Riyadh`, `Asia/Amman` and `Asia/Dubai` with a first period at 07:30 local and a host clock in UTC, when the warm-up of Section 9 is scheduled for a school day, then each tenant's warm-up completes before 07:30 in its own time zone and the `state:warmup:` marker carries the tenant's local date, not the UTC date | REQ-PERF-025, REQ-L10N-010 |
