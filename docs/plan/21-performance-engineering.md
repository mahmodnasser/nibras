...
| `svc_gateway` | `redis-cache`, `redis-state` | `~nibras:platform:platform:tenant-by-host:*` read-only; `~state:ratelimit:*:edge:*` | none | `+@read` on cache; `+@read +@write` on its rate-limit prefix |
...
| 4 | Invoice run chunk: allocate numbers and post 100 invoices (worker, N-03) | `InvoiceRunChunkHandler` | `UPDATE series SET next_number = next_number + 100 ... RETURNING` at read committed; the row lock serialises allocators of one series (`06-services/finance.md` section 4.19) (`10-data-architecture.md` Section 1); invoices and lines inserted by EF Core batching; `account_balances` by `ExecuteUpdateAsync`; one outbox insert for the 100 `finance.invoice.issued.v1`; `ux_series_code` | 100 written / 100 written | chunked by 100 | 5 per chunk | 40 ms | no |
...
```sql
-- Finance: invoices are list-partitioned by academic_year_id and payments range-partitioned by month on received_at (10-data-architecture.md Section 5). Money columns are numeric(19,4); no index is built on an expression over money.
...
CREATE UNIQUE INDEX ux_series_code ON finance.series (tenant_id, code) INCLUDE (next_number);                                                                                                           -- query 4: the row the row-locked allocation updates
...
| # | Query | Handler | Shape and index | Rows demo / load | Pagination | Commands | p95 | Compiled |
|---|---|---|---|---|---|---|---|---|
| 1 | Retrieval: top 8 chunks for a question within the caller's data scope | `RetrieveChunksQuery` | `embeddings` hash partition of the tenant, HNSW on `embedding vector_cosine_ops`, filtered by `tenant_id` and `scope_code = ANY(@scopes)` with iterative scan; `SET LOCAL hnsw.ef_search = 64`; `ix_embeddings_hnsw` | 8 / 8 | top k, none | 3 | 40 ms | no |
...
| 4 | Re-index one source record after it changed | `IndexSourceChangedConsumer` | Inbox insert; `ExecuteDeleteAsync` by `(tenant_id, source_type, source_id)`; batched insert of the new chunks | 4 written / 4 written | none | 4 | 20 ms | no |

```sql
-- Ai: embeddings are hash-partitioned by tenant_id into 8 partitions (10-data-architecture.md Section 5). The HNSW index cannot lead with tenant_id; tenant isolation comes from the partition, the btree filter index, the named filter and the row-level security policy, recorded as the one exception to the tenant-first rule in Decisions in force.
CREATE INDEX ix_embeddings_hnsw ON ai.embeddings USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64);                                                                                -- query 1: approximate nearest neighbours per partition
CREATE INDEX ix_embeddings_tenant_scope ON ai.embeddings (tenant_id, scope_code) INCLUDE (source_type, source_id);                                                                                            -- query 1: the filter the iterative scan applies; also the fallback plan for a small tenant
CREATE INDEX ix_embeddings_source ON ai.embeddings (tenant_id, source_type, source_id);                                                                                                                       -- query 4: delete the chunks of one source
...
Growth: embeddings grow with indexed documents and records, never with Sensitive or level S sources; a 20,000-student group holds about 2 million chunks spread over the partition it hashes to. When a tenant's share of a partition makes the iterative scan read more than 10 times k candidates at the N-07 scale run, the planner falls back to `ix_embeddings_tenant_scope` with an exact distance sort, which is correct at any size and slower only for the oversized tenants.
...
|---|---|---|---|---|
| Schedule and hold correctness | Owned by `10-data-architecture.md`: fake clock across a month boundary and a retention boundary, legal hold present and absent | As stated there | Nightly | `TC-DATA-005` |
...
**The pooled-connection isolation test.** `TC-DATA-001` to `TC-DATA-003` prove the contract with a pool of one. This document adds two variants at production settings:
...
| The Ai HNSW index is the one index that does not lead with `tenant_id` | Section 3.20; master brief Section 19 tenant-first rule | Hash partition by tenant, btree filter index, named filter and row-level security as the isolation | If a later pgvector release supports a leading filter column efficiently, the index is rebuilt with `tenant_id` first and this exception disappears |
...
| Pooling cannot leak a tenant | `TC-DATA-001` to `TC-DATA-003`, `TC-PERF-025`, `TC-PERF-026` in `TenantIsolation.Tests` against PostgreSQL and PgBouncer | Every pull request touching persistence; nightly for all services |
