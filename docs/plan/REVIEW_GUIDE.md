...
Master brief Section 27 lists the decisions, and `docs/project/OPEN_QUESTIONS.md` holds **30 open questions**, each with a default. Questions 27 to 30 were added during the scorecard remediation and **all four are still open**: 27 absence-alert timing, 28 the Tier 1 public API, OneRoster and iCal, 29 whether a student's Wellbeing check-in answer may wait on the phone, and 30 which student count bills a school. `docs/plan/01-questions-and-assumptions.md` gives each question its impact, likelihood, score and risk.
