...
**Group** E · **Requirement areas covered** none directly; risks reference the documents that own them · **Last updated** 2026-09-20 by M. Nasser
...
Thirty-seven risks in five categories. Columns are the same in every table.
...
| RISK-03 | The work breakdown (document 34) reveals scope the roadmap did not plan, and the phase ranges in master brief Section 28 stretch | 4 | 3 | 12 | Document 34 is written before Group F is approved and before Phase 1 starts, so the surprise arrives on paper; slices above three days are split rather than estimated; the roadmap carries ranges, not dates, and the range widens visibly when slices are added; the first two capabilities of Phase 1 are broken down to the day as a calibration sample | Sum of slice ranges for a phase exceeds the top of its Section 28 range; a capability has more than fifteen slices; a slice named after a layer or a table appears | Architect | Phase 0, Phase 1 | Open |
...
| RISK-07 | The Phase 1 critical path slips: Identity, Platform and School block every other service because they all keep reference copies of School data | 3 | 4 | 12 | Contract-first slices: the School directory gRPC contract and the `school.*` events are published in the first capability of Phase 1 so consumers start against the contract; the service template ships in week one; the PDF pipeline is built in Phase 1 as the critical path in master brief Section 28 says | The School directory contract is not published by week four of Phase 1; a Phase 2 service starts with a stubbed directory | Architect | Phase 1 | Open |
...
| **3 Possible** | — | — | RISK-06, RISK-15, RISK-16, RISK-18, RISK-23, RISK-27, RISK-28, RISK-29, RISK-30, RISK-32, RISK-36, RISK-37 | RISK-04, RISK-05, RISK-07, RISK-08, RISK-10, RISK-13, RISK-17, RISK-19, RISK-21, RISK-22, RISK-26, RISK-31, RISK-35 | RISK-14 |
...
| 12 to 14 | 14 | Top-ten pool; reviewed at every phase |
| 8 to 11 | 17 | Reviewed at the named phase; escalated on signal |
| 1 to 7 | 1 (RISK-11) | Reviewed at the named phase |
...
| 6 | RISK-07 | 12 | Phase 1 critical path slips | Publish the School directory gRPC contract and the `school.*` event schemas in week one of Phase 1, before any consumer is written | Architect | Phase 1, week one |
...
Risks scoring 12 that fall outside the ten (RISK-05, RISK-08, RISK-10, RISK-17, RISK-19, RISK-22, RISK-26, RISK-35) keep their mitigations and their review phase; each moves into the ten the moment its signal fires.
...
|---|---|
| Product owner | RISK-01, RISK-02, RISK-04, RISK-05, RISK-06, RISK-23, RISK-27, RISK-28, RISK-29, RISK-30, RISK-31, RISK-32, RISK-35, RISK-36 |
| Architect | RISK-03, RISK-07, RISK-10, RISK-11, RISK-12, RISK-13, RISK-14, RISK-15, RISK-19, RISK-20, RISK-21, RISK-24, RISK-26, RISK-33 |
...
The product owner holds fourteen risks, which is itself the evidence for RISK-01; the delegation in part 5 rank 1 is what shortens that list.
...
| 17 and 34 | The phase ranges and slices behind RISK-02, RISK-03 and RISK-07 |
| 19 | Verified licences behind RISK-11, RISK-14 and RISK-26 |
...
|---|---|---|
| Every seed risk in master brief Section 40 and `RISKS.md` has an identifier here | Part 2 maps all twelve; `/lint-plan` compares the seed table against the Section 40 table and fails on a missing row | `ci-kit.yml` |
| Every risk has a likelihood, an impact, a score equal to their product, a mitigation, a signal, an owner role from master brief Section 29, a review phase and a status | A `/lint-plan` check on the register tables: ten non-empty cells per row, the score cell equals L times I, the owner is one of the five roles, the status is one of the four values | `ci-kit.yml` |
| The heat map and the top ten agree with the register | `/lint-plan` recomputes the cells and the ranking from the register rows and diffs them against parts 4 and 5 | `ci-kit.yml` |
| Every test identifier cited as a mitigation exists | Document 20 carries each `TC-` identifier named in part 3; `/lint-plan` refuses one absent there | `ci-kit.yml` |
