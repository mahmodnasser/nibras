...
| 9. REQ-DOC-002's acceptance says a signed URL fails after 15 minutes; Appendix J and Appendix K say 5 | 5 minutes | Product owner | Longer links widen T-DOC-02 |
| 10. Appendix O references `TC-DOC-002` for the QR verification demo while Appendix W uses it for school memory | This sheet uses `TC-DOC-002` for school memory and `TC-DOC-302` for verification | Appendix O owner | A renumbered demo step |
| 11. Public uploads from the admissions form (REQ-DOC-009 example) arrive before any account exists | Admissions issues a short-lived upload token after OTP and proxies the upload to `POST /files` with its service identity | Admissions lead | A public upload endpoint here would need its own bot protection |
| 12. School memory needs the media consent flag, which School owns | The requesting service confirms consent per item at compile time; Documents stores `consent_checked_at` | School lead | A consent copy here would duplicate a Confidential field |
