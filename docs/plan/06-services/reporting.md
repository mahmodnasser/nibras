...
These calls run only inside the rebuild job and the nightly consistency sample, never inside a user request, so the one-hop rule holds and no request of Reporting's has a synchronous dependency, as table 8.0 says. Open point 2 records that table 8.0 does not list these job-only reads.
...
Queues are those of `11-messaging-architecture.md` §2.5 for Reporting: `reporting.tenant-lifecycle` and `reporting.commands` in the Api host, and the eight ordered shards `reporting-projections.projections.0` to `.7` behind `nibras.reporting-projections.projections.hash`, one handler concurrency per shard, prefetch 64. Every projection handler writes the inbox row, the projection rows and the checkpoint in one transaction. Keys marked † are named by `10-data-architecture.md` part 7.1 as projection sources but Appendix E's consumer column does not name Reporting for them (Open point 7); they are bound only after that correction.
...
| `academics.assignment.published.v1`, `academics.submission.received.v1`, `academics.submission.graded.v1`, `academics.lesson-plan.submitted.v1`, `academics.homework-load.exceeded.v1`, `academics.syllabus-coverage.behind.v1` | shards | `MarkFactsProjection`, `StaffingFactsProjection`, `EarlyWarningInputsProjection`, `StudentTimelineProjection` | Missing work, graded work, lesson-plan counts, coverage flags | `assignmentId`, `staffId` or `sectionId` plus `occurredAt` |
| `assessment.marks.entered.v1`, `assessment.marks.approved.v1`, `assessment.grades.locked.v1`, `assessment.report-cards.published.v1`; `assessment.marks.overdue.v1` † | shards | `MarkFactsProjection`, `Student360Projection`, `StaffingFactsProjection` | Marks with scheme version, lock state, last report card, overdue grading | `componentId` or `gradingPeriodId` plus `occurredAt` |
...
| WF-SCH-03 Year archival and reopen | Participant | Sealed year snapshot so a rerun gives the same numbers (TC-SCH-026); the Appendix R side effect key for the seal is not an Appendix E key (Open point 9) |
...
Existing identifiers are reused; new ones are minted in `TC-RPT-310` to `TC-RPT-360`, a range no document in the kit uses (checked with a search of `docs/` and `.claude/` on 2026-09-21: existing RPT identifiers are 001 to 008, 102, 201, 202, 501).

| Test case | Level | What it proves |
|---|---|---|
| TC-RPT-001 | UAT, feature register | Morning brief and Today cards, each card with one action |
| TC-RPT-002 | Feature register | Student 360 timeline filtered by the viewer's permissions |
| TC-RPT-003 | Feature register | Early warning with explanation and an intervention; degrades to rules |
| TC-RPT-004 | Feature register | Data Quality Center with fix actions |
| TC-RPT-005 | Feature register | Inspection readiness evidence folder (Tier 2) |
| TC-RPT-006 | Feature register | Morning brief per role before the day starts |
| TC-RPT-007 | Feature register | Explain this number to the records |
| TC-RPT-008 | Feature register | Because panel with reasons, rung and a reasoned override |
...
| TC-DATA-013 | Integration | Replay paused: reads routed to the primary and the alert raised |
| TC-PERF-027 | Load | N-05 with replay paused 60 s: p95 under 250 ms on the primary, routing returns within 30 s |
| TC-SCH-026 | Integration | A historical report rerun after a structure change gives the same numbers from the sealed snapshot |
...
|---|---|---|---|
| 1. REQ-RPT-016 and REQ-RPT-017 (governance, Tier 2) are assigned to Reporting, which "owns no source data" | Keep them in a `governance` schema of `nibras_reporting`, excluded from rebuild, under an ADR; Appendix B has no `reporting.governance.*` resource, so `reporting.reports.*` gates them | Architect, Appendix B owner | Moving them to Operations or Platform moves one schema and eleven endpoints |
| 2. Table 8.0 lists no synchronous dependency for Reporting, but part 7.3 rebuilds from every source's `Snapshot` and Wellbeing's `Counts` | Job-only reads, never on a request path; record them in table 8.0 under an ADR | Architect | Without the ADR the table and this sheet disagree |
| 3. Appendix B has no action for overriding a flag | `reporting.early-warning.open-intervention` gates both decisions on a flag | Appendix B owner | A `reporting.early-warning.override` action would replace it |
...
| 7. `10-data-architecture.md` part 7.1 lists sources that Appendix E does not route to Reporting: `school.guardian.updated.v1`, `admissions.offer.made.v1`, `assessment.marks.overdue.v1`, `attendance.attendance.not-marked.v1`, `finance.account.restricted.v1`, `finance.account.cleared.v1`, `documents.export.completed.v1`, `scheduling.substitution.assigned.v1`, `audit.integrity-check.failed.v1` | Add Reporting to those consumer columns under a version bump; until then the † rows are not bound and their cards read "not available" | Appendix E owner | Cards that need them stay empty |
| 8. Appendix R WF-WEL-05 has Reporting raise a flag from check-in patterns, but no check-in event exists in Appendix E and Reporting may hold no Wellbeing detail | Reporting does nothing for WF-WEL-05; Wellbeing escalates within itself | Appendix R owner | A check-in signal would have to be a count, never a response |
| 9. Appendix R WF-SCH-03 names a `reporting` snapshot-sealed key that is not in Appendix E | Seal on `school.academic-year.closed.v1` and record `reporting.audit.recorded.v1`; no new key | Appendix E owner | School cannot wait on the seal without the key |
