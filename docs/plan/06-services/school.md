...
| `school.student.enrolled.v1` | `studentId`, `studentNumber`, `sectionId`, `campusId`, `enrolledOn`, `namesEnAr` | `studentId` | `CreateStudentHandler`, `EnrolStudentHandler` (Saga 3 step 2, re-published with the same `studentId` on a duplicate), Saga 9 commit, `ReverseWithdrawalHandler` | Academics, Assessment, Attendance, Finance, Communication, Behavior, Operations, Reporting, Requests, Admissions, Notification; Platform for the billable active-student count (BR-FIN-017, ADR-0027) |
...
| `school.student.status-changed.v1` | `studentId`, `fromStatus`, `toStatus`, `effectiveOn`, `reasonCode` | `studentId` | Every `Student.ChangeStatus` transition: status change, withdrawal step 5, rollover for graduates and leavers, `WithdrawEnrolment` compensation (`toStatus = never-attended`), merge victim | Academics, Assessment, Attendance, Finance, Communication, Behavior, Operations, Requests, Wellbeing, Reporting, Ai; Admissions as a Saga 3 outcome; Platform for the leaving date of the billable active-student count (BR-FIN-017, ADR-0027) |
...
| 2026-09-26 | Open Question 30 decided (ADR-0027) | Section 7.1 names Platform as a consumer of `school.student.enrolled.v1` and `school.student.status-changed.v1`: Platform keeps `ref_student_enrolment_periods` from them and computes the billable active-student count (BR-FIN-017); School's hourly `active-students` usage fact is the daily figure Platform reconciles that copy against |
