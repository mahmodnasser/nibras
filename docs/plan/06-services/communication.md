...
| 7 | `communication.message.reported.v1` payload lacks the conversation's student subject, which Wellbeing needs to open a case | Wellbeing receives `messageId` and asks the safeguarding officer to link the student in its own case screen | Architect, with Wellbeing | One manual step per case | 3 | 1 | 3 | none |
