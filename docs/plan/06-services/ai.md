...
| 2 | Master brief Section 25 says "a change event re-indexes the record", but Appendix E lists Ai as a consumer only of `school.student.status-changed.v1`; `school.student.section-changed.v1` and the source change events of document 25 §4.2 are not bound (document 11 open point 7) | The five source change keys and `school.student.section-changed.v1` are bound on `ai.index-sources.bulk` and queue indexing at once; the record is still read through the Bff.Web feed; `IndexSyncJob` is the catch-up path and `IndexReconciliationJob` the nightly proof | Product owner, Appendix E amendment | Resolved 2026-09-22 (ADR-0019): Appendix E added Ai to the consumers of `communication.announcement.published.v1`, `assessment.report-cards.published.v1`, `academics.lesson-plan.submitted.v1`, `attendance.attendance.marked.v1`, `behavior.incident.recorded.v1` (restricted incidents skipped) and `school.student.section-changed.v1`, which answers document 11 open point 7 yes and gives Ai the queue that open point named. Master brief Section 25 now holds |
| 3 | Appendix K.21 gives `AI_USAGE_LIMIT_REACHED` HTTP 402 and `AI_DISABLED_FOR_TENANT` a hard 403, while document 25 §8 degrades both to rung 1 | Disabled feature: 403 so the client hides the entry point; limit reached: 200 degraded carrying the code, with 402 only on an administrator's explicit rung 4 request | Product owner | Resolved 2026-09-22 (ADR-0019): K.21 now reads "200 on assist requests; 402 only on a rung 4 request" and names the two configuration routes that can answer 402, and `AI_DISABLED_FOR_TENANT` keeps the 403 this sheet defaulted to. The ADR left `AI_MODEL_UNAVAILABLE` at 503 because no logged defect covered it, so section 11.4 records that this sheet returns it as a degradation reason on assist endpoints and the two disagree until the next Appendix K revision |
| 4 | Appendix G's AI group has no key for the template version, the model tag or the rung 4 field groups per feature | The three keys are Appendix G AI settings; their values live in `feature_configurations` and `provider_configurations` and are edited under `ai.configuration.configure` and `ai.configuration.set-provider` | Product owner | Resolved 2026-09-22 (ADR-0019): Appendix G's AI group carries template version per feature (default the latest version shipped with the release that passed the harness), model tag per feature (default the deployment's pinned model) and rung 4 field groups (default none, never Sensitive or level S, a change needs a new tenant consent), so Platform owns the definitions and Ai the values (ADR-0009) |
| 5 | Appendix K.21 says a blocked injection notifies the administrator, but Appendix C has no Ai row and document 11 §2.4 does not list `nibras.ai` among the senders of `RequestNotification` | The daily digest runs on the Appendix C row "AI injection attempts blocked" and the Appendix E job that triggers it; the `nibras.ai` binding into `notification.commands` is still needed | Document 11 owner, product owner | Partly resolved 2026-09-22 (ADR-0019): Appendix C added the row, *(T2)*, to the school and security administrators, urgency N, email and in-app, never the planted text, and Appendix E's job table added the daily per-tenant Ai digest. ADR-0019 recorded the `nibras.ai` sender binding as plan-level work for document 11, so until it lands the administrator still learns of attempts only from the audit viewer |
...
| 7 | Document 21 §1.20 and §3.20 and document 10 §5 named the table `ai.embeddings` with `scope_code`, `source_type` and index `ix_embeddings_hnsw`, while document 25 §4.1 names `ai_index.embedding_chunk` with array scope tags and `ix_chunk_*` | Appendix F settles the name as `ai_index.embedding_chunk`; document 21's query rows apply to that name | Architect | Closed 2026-09-22 (ADR-0019, then scorecard theme 4). Appendix F's Ai line names `IndexedChunk` with table `ai_index.embedding_chunk`, and documents 10 §5 and 21 §1.20 and §3.20 now carry that name and the `ix_chunk_*` indexes, so a budget test generated from document 21 targets the table this sheet builds |
...
| 9 | Appendix F lists no Ai entity | Appendix F's Ai line is the entity list and matches section 4 of this sheet | Architect | Resolved 2026-09-22 (ADR-0019): Appendix F added the Ai line with IndexedChunk (table `ai_index.embedding_chunk`), SourceCheckpoint, PurgedSubject, IndexRebuild, AssistJob and Payload, FeatureConfiguration, ProviderConfiguration, ProviderConsent, CallLogEntry, UsagePeriodTotal and ModelRelease, the names section 4 uses |
