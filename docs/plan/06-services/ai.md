...
| The index drifts from its sources | med | med | Change keys on `ai.index-sources.bulk` index at once, `IndexSyncJob` catches missed messages, nightly reconciliation, rebuild on mismatch | Architect |
...
| The index follows document 25 §4.1 (`ai_index.embedding_chunk` with array scope tags), hash-partitioned as document 10 §5 states | Document 25 owns the index design; Appendix F names the table `ai_index.embedding_chunk` | As stated; document 21 §3.20 names are read as aliases (open point 7) | Two table shapes would reach the migration |
...
| Queues, worker commands, KEDA bounds, the source-change bindings on `ai.index-sources.bulk` | `11-messaging-architecture.md` §2.3 to §2.6, §7, open point 7 | Group C review |
...
| 5 | Appendix K.21 says a blocked injection notifies the administrator, but Appendix C has no Ai row and document 11 §2.4 does not list `nibras.ai` among the senders of `RequestNotification` | The daily digest runs on the Appendix C row "AI injection attempts blocked" and the Appendix E job that triggers it; the `nibras.ai` binding into `notification.commands` is still needed | Document 11 owner, product owner | Partly resolved 2026-09-22 (ADR-0019): Appendix C added the row, *(T2)*, to the school and security administrators, urgency N, email and in-app, never the planted text, and Appendix E's job table added the daily per-tenant Ai digest. ADR-0019 recorded the `nibras.ai` sender binding as plan-level work for document 11, so until it lands the administrator still learns of attempts only from the audit viewer | 2 | 2 | 4 | none |
