...
| Synchronous dependency | none as gRPC (table 8.0); reads through Bff.Web's three internal routes over REST, from its jobs only (section 6; `06-services/bff-web.md` §4.1) |
...
| Delivering any message, including the administrator alert on a blocked injection | Notification | Ai sends `RequestNotification`, which `notification.commands` binds on `nibras.ai` (`11-messaging-architecture.md` §2.5) |
...
**Consumed: none.** Table 8.0 (v9.1) gives Ai "none as gRPC; reads through Bff.Web's three internal routes over REST, from its jobs only". Those three routes are `GET /bff/web/v1/internal/ai/sources/{sourceEntity}?cursor=` (service credential), `POST /bff/web/v1/internal/ai/sources/authorize` and `POST /bff/web/v1/internal/ai/tools/{toolName}` (the caller's token), exactly as section 5 and section 9 use them, and the Bff.Web row of the same table records that Bff.Web submits assist jobs to Ai over REST and serves those three routes. Model calls answer 202. Ai's only other outbound calls are to the model servers; none of this is a service-to-service gRPC hop.
...
Worker jobs Ai sends to itself on `nibras.ai` (document 11 §2.4, §2.5): `ai.commands.index-source.v1` and `ai.commands.rebuild-index.v1` into `ai-worker.embeddings.bulk`, and `ai.commands.draft.v1` into `ai-worker.drafts` for draft and translate jobs. Commands and replies Ai sends to others: `RequestNotification` (`notification.commands.request-notification.v1`) for the daily administrator digest of blocked injections, which Notification's `notification.commands` queue binds on `nibras.ai` (document 11 §2.5), and the tenant-lifecycle replies to Platform. The cross-cutting `<service>.usage.recorded.v1` is `ai.usage.recorded.v1` itself; Ai publishes no second meter.
...
| `communication.announcement.published.v1`, `assessment.report-cards.published.v1`, `academics.lesson-plan.submitted.v1`, `attendance.attendance.marked.v1`, `behavior.incident.recorded.v1` | `ai.index-sources.bulk` | `SourceChangedConsumer` | Queues one `ai.commands.index-source.v1` for the changed record on its stream, keyed on the record and its version; a stream that is paused or whose feature is off is skipped; a restricted behavior incident is never queued (document 25 §4.2) |
| `school.student.section-changed.v1` | `ai.index-sources.bulk` | `StudentSectionChangedConsumer` | Re-tags the student's chunks with the new section scope without re-embedding, keyed on `studentId` plus `effectiveOn` |
...
Appendix E (v9.1) names Ai in the consumer column of the five source change keys and of `school.student.section-changed.v1`, which answers document 11 open point 7 in the affirmative and gives Ai the `ai.index-sources.bulk` queue that open point named. A change key queues the record for indexing at once; the record itself is still read through the Bff.Web source feed, so no payload of an indexed source ever travels on the bus. `IndexSyncJob` stays as the catch-up path for a missed or parked message, and `IndexReconciliationJob` stays as the nightly proof.
...
|---|---|---|---|---|---|
| `IndexSyncJob` | Api | Every 15 minutes, 06:00 to 20:00 tenant time zone; hourly outside | Catch-up for records the change keys of section 7.2 did not deliver: per unpaused stream, pulls changed records since the cursor from the Bff.Web feed and queues one `ai.commands.index-source.v1` per record; scope changes re-tag without re-embedding | `ai.commands.index-source.v1` | none |
...
Appendix I (v9.1) carries these permissions as group **G26 Assist (Ai)**: `ai.assistant.use`, `ai.drafting.use`, `ai.drafting.accept-draft`, `ai.configuration.set-provider` and `ai.usage.view`, elevated at `set-provider`. The School Owner, Principal, Academic Coordinator, Head of Department, Registrar, HR Officer, Teacher, Admissions Officer and Accountant hold it for use; the Vice Principal holds use without `ai.configuration.*`; IT Support holds `ai.usage.view` only. The Counselor, the Special-Needs Coordinator, the Safeguarding Officer, the Nurse and the Platform Administrator list `ai.*` as must-not, because the index refuses Sensitive and level-S data. Permission groups now run G01 to G26, since Appendix I also gained G25 Notification.

### 11.2 Notifications (Appendix C)

Appendix C (v9.1) carries one Ai row, **AI injection attempts blocked** *(T2)*, triggered by the daily digest of blocked attempts and listed in Appendix E's job table as a daily per-tenant Ai job. It goes to the school administrator and the security administrator, urgency N, by email and in-app, and never carries the planted text. The digest is sent with `RequestNotification`; document 11 §2.5 lists `nibras.ai` among the exchanges `notification.commands` binds, so the command reaches Notification (open point 5, closed). Rebuild completion is shown on the Platform console from `ai.index.rebuild-completed.v1`, not notified.
...
Appendix G's AI group (v9.1) adds **template version per feature** (default the latest version shipped with the release that passed the evaluation harness), **model tag per feature** (default the deployment's pinned model) and **rung 4 field groups** (default none, never a Sensitive or level-S group, and a change needs a new tenant consent). Their values are held in `feature_configurations` and `provider_configurations` and edited under `ai.configuration.configure` and `ai.configuration.set-provider`, so Platform owns the definitions and Ai owns the values (ADR-0009).
...
| `AI_USAGE_LIMIT_REACHED` | 200 on assist requests; 402 only on a rung 4 request | As `degradedReason` of a rung 1 outcome on every assist endpoint (section 5 degradation rule); the 402 is returned only to an administrator routing a feature to the external provider with the plan's rung 4 allowance spent, that is `PUT /api/v1/ai/configuration/features/{featureCode}` with `preferExternal: true` or `PUT /api/v1/ai/configuration/provider` |
| `AI_MODEL_UNAVAILABLE` | 200 degraded on assist endpoints; Appendix K.21 still records 503 | As `degradedReason` when the circuit is open or the model times out; never an HTTP error on an assist endpoint (REQ-AI-008, master brief Section 25 "fall back downward"). ADR-0019 left the K.21 row at 503 because no logged defect covered it, so the two disagree until the next Appendix K revision |
...
The caching map is `21-performance-engineering.md` §1.20 and the hot queries are its §3.20; both are binding, and both use the table and index names of document 25 §4.1 (`ai_index.embedding_chunk`, `ix_chunk_scope`, `ix_chunk_vector`), so no alias is needed (open point 7, closed). This sheet adds:
...
│   │   ├── SourceChangedConsumer.cs                                            the five source change keys on ai.index-sources.bulk
│   │   └── StudentSectionChangedConsumer.cs                                    school.student.section-changed.v1 re-tags scope, no re-embed
...
│   │   ├── AiTopology.cs                                                       exchange nibras.ai; queues ai.tenant-lifecycle, ai.commands, ai.events, ai.index-sources.bulk, ai-worker.embeddings.bulk, ai-worker.drafts
...
| `TC-PLT-026` (Appendix R) | `platform.tenant.deleted.v1` deletes every Ai row of the tenant | Integration |
...
| TC-AI-636 | A student's section change reaches the index through `school.student.section-changed.v1` on `ai.index-sources.bulk` as a re-tag with no re-embedding, and, with that message parked, through the next `IndexSyncJob` run instead | Integration, `ScopeRetagTests` |
...
| TC-AI-750 | The same Arabic and English source records chunked and slotted with the process culture set to `ar-SA`, then `en-US`, then the invariant culture give byte-identical chunks, `content_folded` values, token counts and template slots, and a date slot is ISO 8601 in the Gregorian calendar with Latin digits, never the Um Al Qura form `ar-SA` formats by default | Unit, `ChunkerTests` and `PromptAssembler` tests, on `ubuntu-latest` |
| TC-AI-751 | An Arabic source whose text carries the bidirectional controls U+202E, U+2066 and U+2069 is embedded and slotted without them, a retrieved chunk carrying them cannot reorder the labelled data blocks of the prompt, and an Arabic draft returned to the requester contains none; a Latin student name and a numeral inside the Arabic draft keep their stored order | Unit and integration, `ChunkerTests`, `OutputFilterTests` |
...
|---|---|---|
| Runner | Unit, integration and contract suites run on the Linux runner (`ubuntu-latest`) with PostgreSQL and pgvector, RabbitMQ, Redis and a stub model server in Testcontainers; the `ai-eval` stage and the injection fixtures run on Linux with the pinned local model of the `ai` compose profile. Ai is not among the projects document 33 part 4 runs on Windows (`BuildingBlocks`, `Documents`, `Localization`). `dev-smoke` on `ubuntu-latest`, `windows-latest` and `macos-latest` starts the stack without the `ai` profile, because every feature is off by default and the product is complete without Ai (ADR-0015) | Document 33 part 4; `TC-AI-601` for the all-off product |
| Culture-sensitive values | Chunk text, folded text, token counts, slots and the call log are machine-facing and built under the invariant culture; Arabic folding on the lexical leg of ranking is `nibras_ar_fold`, the same function the owning services use (BR-L10N-001) | `TC-AI-750`; `TC-L10N-310` (document 24) inside the database image |
| Right-to-left and bilingual output | Drafts, translations and answers are text returned to the web client, which sets direction (document 08); this service only guarantees the text holds no bidirectional control that could reorder it, and that Arabic quality is held to the English threshold | `TC-AI-751`, `TC-AI-611`, `TC-AI-638` |
| Devices without Google services | Not relevant: nothing Ai stores or produces reaches a device (section 13), and every Ai surface is on the web | Not applicable |

**Signature features.** Ai owns none. It is the rung 3 path of Appendix W features 6 (report-card comments at rung 3, autonomy 3), 11 (natural-language search at rung 3), 25 (morning brief phrasing at rung 3) and 28 (the Because panel it fills for rungs 3 and 4), each of which is complete at rung 1 without it. Each is cited by its Appendix W number; its moment, rung, autonomy, the requirements and slices that build it, its Appendix O step and its demo test are held once, in the "Signature feature trace" table of `32-product-differentiation-and-demo.md`, and are not copied here.
...
|---|---|---|---|---|---|---|
| Retrieval returns a row outside the caller's scope | 3 | 4 | 12 | Filter before rank, per-source re-check, generated isolation suite; TC-AI-607, TC-AI-630, TC-SEC-321 | Ai lead | RISK-38 |
| A planted instruction makes the assistant act | 4 | 4 | 16 | Data blocks, no write tools, tools under the caller's token, fourteen fixtures in both languages; TC-SEC-320 | Security reviewer | RISK-38 |
| A withdrawn student's content remains retrievable | 3 | 4 | 12 | Purge in the consumer's transaction, `PurgedSubject` guard on writes, nightly audit; TC-AI-605 | Ai lead | RISK-38 |
| Student data leaves the school without consent | 2 | 4 | 8 | Rung 4 off by default, consent gate at submit and run, egress allow-list; TC-SEC-323, TC-AI-602 | Product owner | none |
| A model outage turns into errors or timeouts across screens | 3 | 3 | 9 | Circuit state from the probe, 15 s cap, rung 1 fallback, never an error; TC-AI-608, TC-AI-637 | Ai lead | none |
| Arabic quality regresses behind English | 3 | 3 | 9 | Release blocked on the parity threshold; human raters per release; TC-AI-638 | Ai lead with the Arabic reviewer | RISK-05 |
| The index drifts from its sources | 3 | 3 | 9 | Change keys on `ai.index-sources.bulk` index at once, `IndexSyncJob` catches missed messages, nightly reconciliation, rebuild on mismatch; TC-AI-636 | Architect | none |
...
| The source change keys of Appendix E queue indexing on `ai.index-sources.bulk`; the record is still read through the Bff.Web source feed | Appendix E (v9.1) consumer columns; ADR-0019 | As stated | Carrying source content on the bus would put indexed text outside the owning service |
| The index follows document 25 §4.1 (`ai_index.embedding_chunk` with array scope tags), hash-partitioned as document 10 §5 states | Document 25 owns the index design; Appendix F names the table `ai_index.embedding_chunk`, and documents 10 §5 and 21 §1.20 and §3.20 use the same names | As stated | Two table shapes would reach the migration |
| `IAssistGateway` is a contract in `Nibras.Contracts.Ai` implemented over REST for Bff.Web, not a gRPC service | Reference architecture Section 8.0 (v9.1): Ai has none as gRPC and reads through Bff.Web's three internal routes over REST, from its jobs only | As stated | A gRPC surface would add a proto and a second transport to secure |
...
| Model, limit and consent conditions degrade to a 200 rung 1 outcome rather than an HTTP error on an assist endpoint; only an administrator's explicit rung 4 request answers 402 | Master brief Section 25 "fall back downward"; document 25 §1.4; Appendix K.21 (v9.1) | As stated | Clients would show error toasts where the rule requires a template |
...
| Queues, worker commands, KEDA bounds, the source-change bindings on `ai.index-sources.bulk`, `nibras.ai` among the senders of `RequestNotification` | `11-messaging-architecture.md` §2.3 to §2.5, §7 (its open point 7 closed with the binding) | Group C review |
...
| Bff.Web as the only caller and the read path: the `/bff/web/v1/ai/{path}` pass-through, the natural-language search routes and the three internal routes | `06-services/bff-web.md` §4.1, §4.2 | Group C review; `TC-BFF-750` to `TC-BFF-753` |
...
|---|---|---|---|---|---|---|---|---|
| 1 | Document 25 §3.1 says callers reach `IAssistGateway` over gRPC and §4.4 step 5 re-checks each source through the owning service's gRPC check, while table 8.0 gives Ai no synchronous dependency and `06-services/bff-web.md` lists no internal routes for Ai (source feed, source re-check, read tools) | REST from Bff.Web to Ai; Ai calls the three Bff.Web internal routes `GET /bff/web/v1/internal/ai/sources/{sourceEntity}?cursor=` (service credential), `POST /bff/web/v1/internal/ai/sources/authorize` and `POST /bff/web/v1/internal/ai/tools/{toolName}` (caller's token), from its jobs only | Architect, Bff.Web owner | Resolved 2026-09-22 (ADR-0019): reference architecture Section 8.0 now gives Ai "none as gRPC; reads through Bff.Web's three internal routes over REST, from its jobs only", names the three routes exactly as above with model calls answering 202, and records on the Bff.Web row that Bff.Web submits assist jobs to Ai over REST and serves them. Closed 2026-09-26: `06-services/bff-web.md` §4.1 now carries the three internal routes with their credential and budget, the `/bff/web/v1/ai/{path}` pass-through, and a natural-language search that answers 202 with this service's job (`TC-BFF-750` to `TC-BFF-753`), and the Gateway refuses `/bff/web/v1/internal/` from outside (`TC-GW-751`) | 1 | 1 | 1 | none |
| 2 | Master brief Section 25 says "a change event re-indexes the record", but Appendix E lists Ai as a consumer only of `school.student.status-changed.v1`; `school.student.section-changed.v1` and the source change events of document 25 §4.2 are not bound (document 11 open point 7) | The five source change keys and `school.student.section-changed.v1` are bound on `ai.index-sources.bulk` and queue indexing at once; the record is still read through the Bff.Web feed; `IndexSyncJob` is the catch-up path and `IndexReconciliationJob` the nightly proof | Product owner, Appendix E amendment | Resolved 2026-09-22 (ADR-0019): Appendix E added Ai to the consumers of `communication.announcement.published.v1`, `assessment.report-cards.published.v1`, `academics.lesson-plan.submitted.v1`, `attendance.attendance.marked.v1`, `behavior.incident.recorded.v1` (restricted incidents skipped) and `school.student.section-changed.v1`, which answers document 11 open point 7 yes and gives Ai the queue that open point named. Master brief Section 25 now holds | 1 | 2 | 2 | none |
| 3 | Appendix K.21 gives `AI_USAGE_LIMIT_REACHED` HTTP 402 and `AI_DISABLED_FOR_TENANT` a hard 403, while document 25 §8 degrades both to rung 1 | Disabled feature: 403 so the client hides the entry point; limit reached: 200 degraded carrying the code, with 402 only on an administrator's explicit rung 4 request | Product owner | Resolved 2026-09-22 (ADR-0019): K.21 now reads "200 on assist requests; 402 only on a rung 4 request" and names the two configuration routes that can answer 402, and `AI_DISABLED_FOR_TENANT` keeps the 403 this sheet defaulted to. The ADR left `AI_MODEL_UNAVAILABLE` at 503 because no logged defect covered it, so section 11.4 records that this sheet returns it as a degradation reason on assist endpoints and the two disagree until the next Appendix K revision | 2 | 1 | 2 | none |
| 4 | Appendix G's AI group has no key for the template version, the model tag or the rung 4 field groups per feature | The three keys are Appendix G AI settings; their values live in `feature_configurations` and `provider_configurations` and are edited under `ai.configuration.configure` and `ai.configuration.set-provider` | Product owner | Resolved 2026-09-22 (ADR-0019): Appendix G's AI group carries template version per feature (default the latest version shipped with the release that passed the harness), model tag per feature (default the deployment's pinned model) and rung 4 field groups (default none, never Sensitive or level S, a change needs a new tenant consent), so Platform owns the definitions and Ai the values (ADR-0009) | 1 | 1 | 1 | none |
| 5 | Appendix K.21 says a blocked injection notifies the administrator, but Appendix C has no Ai row and document 11 §2.4 does not list `nibras.ai` among the senders of `RequestNotification` | The daily digest runs on the Appendix C row "AI injection attempts blocked" and the Appendix E job that triggers it; the `nibras.ai` binding into `notification.commands` is still needed | Document 11 owner, product owner | Resolved 2026-09-22 (ADR-0019): Appendix C added the row, *(T2)*, to the school and security administrators, urgency N, email and in-app, never the planted text, and Appendix E's job table added the daily per-tenant Ai digest. Closed 2026-09-26: the plan-level half is done, because document 11 §2.5 binds `notification.commands` on `nibras.ai` for `RequestNotification`, so the digest reaches the administrators | 1 | 1 | 1 | none |
| 6 | Appendix I grants no `ai.*` permission in any role template | Group G26 Assist (Ai) holds `ai.assistant.use`, `ai.drafting.use`, `ai.drafting.accept-draft`, `ai.configuration.set-provider` and `ai.usage.view`, granted as section 11.1 lists | Product owner | Resolved 2026-09-22 (ADR-0019): Appendix I added G25 Notification and G26 Assist, granted G26 across the tenant templates, gave IT Support `ai.usage.view` only, listed `ai.*` as must-not for the roles whose work sits at isolation level S, and added the I.4 matrix for both. Permission groups now run G01 to G26 | 1 | 1 | 1 | none |
| 7 | Document 21 §1.20 and §3.20 and document 10 §5 named the table `ai.embeddings` with `scope_code`, `source_type` and index `ix_embeddings_hnsw`, while document 25 §4.1 names `ai_index.embedding_chunk` with array scope tags and `ix_chunk_*` | Appendix F settles the name as `ai_index.embedding_chunk`; document 21's query rows apply to that name | Architect | Closed 2026-09-22 (ADR-0019, then scorecard theme 4). Appendix F's Ai line names `IndexedChunk` with table `ai_index.embedding_chunk`, and documents 10 §5 and 21 §1.20 and §3.20 now carry that name and the `ix_chunk_*` indexes, so a budget test generated from document 21 targets the table this sheet builds | 1 | 1 | 1 | none |
| 8 | Document 25 §3.1 and §7 have rung 2 models in Reporting, Assessment and Hr emit `ai.usage.recorded.v1` and `ai.suggestion.rejected.v1`, but document 11 §1.4 lets a service publish only on its own exchange | Ai publishes both keys for rungs 3 and 4 only; rung 2 metering is each owner's `<service>.usage.recorded.v1` with meter `ai-rung2`, and a rung 2 override stays in the owner's `OverrideRecord`; a Reporting override event stays proposed | Architect, Reporting owner | Still open. ADR-0019 added no Reporting override key to Appendix E, so Reporting's rejection counts for rung 2 have no event until the amendment | 3 | 2 | 6 | none |
| 9 | Appendix F lists no Ai entity | Appendix F's Ai line is the entity list and matches section 4 of this sheet | Architect | Resolved 2026-09-22 (ADR-0019): Appendix F added the Ai line with IndexedChunk (table `ai_index.embedding_chunk`), SourceCheckpoint, PurgedSubject, IndexRebuild, AssistJob and Payload, FeatureConfiguration, ProviderConfiguration, ProviderConsent, CallLogEntry, UsagePeriodTotal and ModelRelease, the names section 4 uses | 1 | 1 | 1 | none |
| 10 | Which open-weight chat and embedding models are pinned for Arabic | As document 25 open point 2: the best scorer on the golden sets among licence-compatible models at the Ai phase, recorded by ADR | Ai service owner | A weaker Arabic model ships | 3 | 3 | 9 | RISK-38 |
| 11 | Open question 5 of `01-questions-and-assumptions.md` is still open: is local AI hardware available, or does assist ship later? The build phase in this sheet's fact table depends on the answer | Rungs 1 and 2 ship inside their owning services; rung 3 ships later, off by default per tenant and per feature, and Ai is built in phase 5 (ADR-0015) | Product owner (open question 5) | Every rung 3 feature stays on its rung 1 or 2 path and the assist ladder cannot be shown in the phase 5 demo; the product itself stays complete without Ai | 3 | 2 | 6 | RISK-39 |
...
| Indexing and prompts do not depend on the host culture, and no bidirectional control survives into a prompt or a draft | TC-AI-750, TC-AI-751 on `ubuntu-latest` | Every pull request touching Ai |
| The source change keys reach the index | TC-AI-636 through `ai.index-sources.bulk`; the bindings themselves are a `messaging-reviewer` review step against document 11 §2.5 at the Group C review | Integration suite; review |
