...
| `documents.document.generated.v1` | `operations.events` (document 11 §2.5) | `DocumentGeneratedConsumer` | Writes `documentId` into `document_id` on the purchase order or safety record the `GenerateDocument` was sent for; a reply for a participant list or the depreciation file, which keep no id, changes nothing, and a `subjectId` Operations does not own is ignored; keyed on `documentId` |
