...
The booking cancellation notice of BR-SCD-007 has no Appendix C row and goes through `RequestNotification` on `nibras.scheduling`, as uncatalogued messages do in the other sheets, which document 11 §2.5 binds into `notification.commands`: `PublishTimetableHandler` writes one command per auto-cancelled booking to the outbox in the publish transaction, with a Scheduling template code, the requester as recipient, the booking id and the version that cancelled it, and no other content; normal urgency, in-app and push (TC-SCD-136, open point 9).
...
│   │   │   ├── PublishTimetableHandler.cs                               BR-SCD-006, booking auto-cancel and its RequestNotification, outbox
...
| TC-SCD-135 | `POST /timetable-versions/{id}/document` for one section's week answers 202 and writes one `GenerateDocument` to the outbox with the resolved week grid, the language and the section as `subjectId`; the same `Idempotency-Key` again sends nothing new; a subject outside the caller's data scope is `SCHEDULING_PERMISSION_DENIED` and an unknown version `SCHEDULING_NOT_FOUND` (REQ-SCD-012) | Integration |
| TC-SCD-136 | Publishing a version that collides with 2 approved bookings auto-cancels both and writes 2 `RequestNotification` commands in the publish transaction, each to its requester with the booking id and the version; a rolled-back publish sends none; a publish replayed with the same `Idempotency-Key` sends none (BR-SCD-007) | Integration |
...
| 9 | The booking cancellation notice of BR-SCD-007 has no Appendix C row. The sender half is closed 2026-09-26: `11-messaging-architecture.md` §2.4 lists `nibras.scheduling` as a `RequestNotification` sender and §2.5 binds it into `notification.commands`, so the uncovered-period alert of TC-HR-004 is delivered by push (the Appendix C row and its job are settled, see above). The delivery half is closed in round 6: `PublishTimetableHandler` sends the cancellation notice through `RequestNotification` (section 11, TC-SCD-136). Still open: the Appendix C row | Send the notice through `RequestNotification` with a Scheduling template code, normal urgency, in-app and push; propose the Appendix C row with the Hr and Operations rows (their open point 2) | Appendix C owner, product owner | The notice is sent, but has no catalogued urgency or channel and a tenant cannot see it in the notification catalogue | 2 | 1 | 2 | RISK-43 |
...
| 2026-09-26 | Round-5 scorecard, remediation round 6 | Section 5.3 cites document 11 §2.5 `notification.events.bulk` (not §2.3) for the ready notice, and TC-SCD-135 for `POST /timetable-versions/{id}/document`; the BR-SCD-007 cancellation notice is sent through `RequestNotification` by `PublishTimetableHandler` (sections 7.1 and 11, the tree, TC-SCD-136), and open point 9 is narrowed to the Appendix C row and re-scored 2 x 1. Awaiting Group C re-review |
