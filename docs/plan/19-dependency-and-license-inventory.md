...
| No disallowed licence reaches a build | `tools/license-scan/run.mjs` over NuGet, npm and pub with the fixes recorded in §11.3; `TC-SEC-393` (REQ-SEC-012) seeds a BSL package and an `allow.json` entry without `adr` and expects both to fail | Every pull request |
...
| This document agrees with the catalogs | `tools/kit-lint` for section and appendix references and forbidden words; `/lint-plan` for consistency with Section 6, reference architecture Section 16 and `12-security-privacy-safety.md` | Every change under `docs/` |
