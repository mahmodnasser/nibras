...
**Group** C · **Requirement areas covered** MSG, plus the messaging rows of PERF, INF and TST · **Last updated** 2026-09-26 by the platform plan
...
| the requesting service (`documents.document.generated.v1`) | the services that send `GenerateDocument` or publish a generation request and keep the returned `documentId`: Platform, Admissions, School, Assessment, Finance, Requests, Hr, Behavior, Operations (the `document_id` on purchase orders and safety records, `06-services/operations.md` §4.3 and §4.4). Scheduling also sends `GenerateDocument` (section 2.4) but keeps no document id: the person who asked finds the file in Documents' generated-documents list (`06-services/documents.md` §4.5), and Notification's `notification.events.bulk` binding tells them it is ready | doc 13 sagas 2, 3, 5, 6, 7, 8; `06-services/hr.md` §7 (offer letters, contracts, payslips, service certificates) and `06-services/behavior.md` §6 (award certificates, portfolio books) |
...
**Notification requests from other services.** `notification.notification.requested.v1` is an **event**, not a command: Appendix E lists it in the Notification table with consumers "Notification workers" and partition key `userId`, and under section 1.4 only Notification may publish a key with its own prefix. Doc 13's sagas and the Appendix E jobs table describe steps and jobs as "sending" it; what they send is the **`RequestNotification` command** (`notification.commands.request-notification.v1`) on the sender's own exchange, and Notification's Api host publishes the catalogued event on `nibras.notification` with the lane header. Nothing outside Notification publishes the event, and nothing binds the command as if it were one. The event stays Notification's own, and the lane is set in one place.
...
| Documents | `GenerateDocument`, `RevokeDocument` | `nibras.platform`, `nibras.admissions`, `nibras.school`, `nibras.assessment`, `nibras.finance`, `nibras.requests`, `nibras.hr`, `nibras.behavior`, `nibras.operations`, `nibras.scheduling` | 2, 3, 5, 6, 8; none for Hr (offer letters, contracts, payslips, service certificates), Behavior (award certificates, portfolio books), Operations (purchase orders, drill and incident reports, participant lists, the depreciation file) and Scheduling (timetable PDFs, REQ-SCD-012), which send `GenerateDocument` from their own endpoints and jobs | `documents.document.generated.v1`; `documents.certificate.revoked.v1` |
...
| Notification | `RequestNotification` | `nibras.platform`, `nibras.admissions`, `nibras.school`, `nibras.finance`, `nibras.requests`, `nibras.academics`, `nibras.scheduling`, `nibras.identity`, `nibras.ai`, `nibras.audit`, `nibras.communication`, `nibras.documents`, `nibras.reporting`, `nibras.behavior`, `nibras.wellbeing`, `nibras.hr`, `nibras.operations` | 1, 2, 3, 5, 6, 8; and the Appendix E jobs that publish `notification.notification.requested.v1`: assignment due reminder (Academics), uncovered-period escalation (Scheduling), joining timeouts (Identity), deletion cooling-off reminder, API key expiry reminder, sandbox lifecycle, support SLA and subject request deadline (Platform), re-enrollment window (Admissions), daily digest of blocked injection attempts (Ai, Appendix C "AI injection attempts blocked"), and the `AuditExportJob` notice to the principal, `06-services/audit.md` section 9 (the job) and section 10.2 (T-AUD-03); none for the jobs and endpoints of the seven senders added on 2026-09-26: meeting reminders and the policy chase (`06-services/communication.md` §10), export approval escalation and file expiry reminders (`06-services/documents.md` §9), the morning brief push and scheduled-report delivery (`06-services/reporting.md` §9), incident review, guardian-notice and plan follow-up reminders (`06-services/behavior.md` §6.1 and §9), the missed-dose alert, safeguarding, guardian-contact and consent escalations (`06-services/wellbeing.md` §6.1 and §9), manager reminders, principal escalations and offer expiry (`06-services/hr.md` §7.1 and §10), and holds, waitlist, transport and safety reminders (`06-services/operations.md` §7.1 and §10). A Wellbeing `RequestNotification` carries the template id, the recipient and identifiers only, never wellbeing content: the template says what happened without saying what is wrong (`06-services/wellbeing.md` §6.1, BR-WEL-003) | `notification.notification.requested.v1`, then `notification.notification.delivered.v1` |
...
| Assessment, worker job | `ComputeResults` | `nibras.assessment`, by the Assessment Api | 7, step 2; and on `ComputePromotionDecisions` | no reply; `Assessment.Worker` takes it from `assessment-worker.results.bulk` and writes result rows with a checkpoint |
| Scheduling, worker job | `SolveTimetable` | `nibras.scheduling`, by the Scheduling Api | none; the generate endpoints of the Scheduling sheet | no reply; `Scheduling.Worker` runs the solve from `scheduling-worker.solve.bulk` |
| Finance, worker job | `RunInvoiceBatch` | `nibras.finance`, by the Finance Api, one message per slice of 200 students | 8, step 2 | no reply; `Finance.Worker` posts the slice from `finance-worker.invoice-runs.bulk`, `finance.invoice.issued.v1` per invoice |
| Finance, worker job | `RunReminderLadder`, `GenerateStatements` | `nibras.finance`, by the daily `ReminderLadderJob` and the monthly `StatementJob` | none; scheduled jobs | no reply; `finance.invoice.overdue.v1` per rung; `GenerateDocument` per payer |
| Notification, worker job | `SendPush`, `SendEmail`, `SendSms` | `nibras.notification`, by the dispatch stage of `Notification.Worker`, routed through `nibras.notification.lanes` | none; one per recipient and channel | no reply; `notification.notification.delivered.v1` or `notification.notification.failed.v1` |
| Notification, worker job | `BuildDigest` | `nibras.notification`, by `DigestSchedulerJob` every 15 minutes, routed through `nibras.notification.lanes` | none; one per user whose digest is due | no reply; the digest leaves through the channel commands above |
| Documents, worker job | `ScanFile`, `ParseImportFile`, `RunExport`, `RunOcr` | `nibras.documents`, by the Documents Api and its `ScanRetryJob` | 9, the parse that feeds steps 2 to 4 (`ParseImportFile`); none for the others | no reply; a clean file, or quarantine and `documents.file.scan-failed.v1`; staged import rows; a `Ready` export file; OCR text and proposed fields |
| Ai, worker job | `IndexSource`, `RebuildIndex`, `Draft` | `nibras.ai`, by the Ai Api and its indexing jobs | none | no reply; replaced chunks; `ai.index.rebuild-completed.v1`; the draft or translation on its job row |

The rows marked worker job are the section 2.4 worker-job keys: a service sends them to its own worker on its own exchange, only the worker queue of section 2.5 binds them, and no reply answers them. `CommitImportBatch` travels both ways: as a Saga 9 command to the target service, and as the Documents worker job `documents.commands.commit-import-batch.v1`.
...
| `notification.commands` | also on `nibras.admissions`, `nibras.school`, `nibras.finance`, `nibras.requests`, `nibras.academics`, `nibras.scheduling`, `nibras.identity`, `nibras.ai`, `nibras.audit`, `nibras.communication`, `nibras.documents`, `nibras.reporting`, `nibras.behavior`, `nibras.wellbeing`, `nibras.hr`, `nibras.operations` (`RequestNotification`, section 2.4; `nibras.platform` through the `<service>.commands` default). A Wellbeing request carries the template id and the recipient only, never wellbeing content (section 2.4, Notification row) | standard | `sagaId`, or `userId` for a job's request | 16 | C 3 | standard |
...
| `documents.commands` | also on `nibras.admissions`, `nibras.school`, `nibras.assessment`, `nibras.finance`, `nibras.requests`, `nibras.hr`, `nibras.behavior`, `nibras.operations`, `nibras.scheduling`; never on `nibras.wellbeing` until the RISK-48 render-path ADR exists (below) | standard | `sagaId` | 16 | C 3 | standard |
...
**Who may send `GenerateDocument`.** `documents.commands` binds the ten exchanges named in its row, which are every service that produces a document: the six saga senders, and Hr, Behavior, Operations and Scheduling for the documents their own endpoints and jobs render. `nibras.wellbeing` is deliberately not bound. Whether Documents may render level S content (referral packs, signed medication administration records) is decided by an ADR with the privacy officer (RISK-48). Until it exists, Documents refuses to render level S Wellbeing content (`06-services/documents.md` open point 13), and Wellbeing does not wire its `DocumentRequestPublisher` (`06-services/wellbeing.md` open point 11), so nothing is published to an unbound queue. Binding `nibras.wellbeing` here before the ADR is the signal RISK-48 watches for.
...
| `behavior.events` | `documents.document.generated.v1` (the award certificate and portfolio book ids Behavior stores; `DocumentGeneratedConsumer` writes the `documentId` on the award or portfolio book the request was made for) | standard | `subjectId` | 16 | S 4 | standard |
...
| `hr.events` | `documents.import.completed.v1`, `requests.request.approved.v1`, `documents.document.generated.v1` (the offer letter, contract, payslip or service certificate id Hr stores) | standard | `jobId`, `requestId`, `subjectId` | 16 | S 4 | standard |
...
| `operations.events` | `finance.payment.received.v1`, `requests.request.approved.v1`, `documents.document.generated.v1` (the purchase order and safety record document ids Operations stores; `DocumentGeneratedConsumer` writes `document_id` on the row the request was made for and ignores a `subjectId` it does not own) | standard | `invoiceId`, `requestId`, `subjectId` | 16 | S 4 | standard |
...
| `ai.index-sources.bulk` | `communication.announcement.published.v1`, `assessment.report-cards.published.v1`, `academics.lesson-plan.submitted.v1`, `attendance.attendance.marked.v1`, `behavior.incident.recorded.v1` (a restricted incident is never queued), and `school.student.section-changed.v1` (a re-tag of scope, no re-embedding) | bulk | `tenantId`, `gradingPeriodId`, `staffId`, `sectionId`, `studentId` (per key, Appendix E) | 32 | B 5 | standard; a parked change is caught up by `IndexSyncJob` from the feed cursor, so nothing waits on a replay |
...
`platform.feature-flag.changed.v1` and `platform.tenant.deletion-requested.v1`, the two keys `05-service-catalog.md` names for Ai, arrive on `ai.tenant-lifecycle`. Doc 05 also says Ai consumes "the change events of indexed sources", and Appendix E (v9.1, ADR-0019) names Ai in the consumer column of the five source change keys and of `school.student.section-changed.v1`, which `ai.index-sources.bulk` binds. A change key only queues one `ai.commands.index-source.v1` for the changed record; the record itself is still read through Bff.Web's internal source feed (`06-services/bff-web.md` §4.1), so no indexed text travels on the bus, and `10-data-architecture.md` section 6 carries the same binding. The queue is bulk because indexing tolerates minutes (`06-services/ai.md` §9 tolerates 15), and it is unordered: the chunk write applies a record only when its `source_version` is newer, so a reordered or repeated change leaves the newest version (`TC-AI-606`, Ai sheet).
...
| the change events of indexed sources | Ai; no longer an addition, because Appendix E (v9.1) names Ai as a consumer of all six keys that `ai.index-sources.bulk` binds, so they carry no asterisk | `05-service-catalog.md` Ai row; ADR-0019 |
...
**The written replay policy.** Master brief Section 12.1 item 40 (Appendix W feature 40, autonomy 4) says "dead letters replay automatically once their transient cause clears", and REQ-PLT-037 builds it as Platform's `FailedMessageAutoReplayJob` (`06-services/platform.md` §10, `TC-PLT-122`). A replay without a person is allowed only inside these bounds, so that it stays a bounded retry rather than an open one:

| Bound | Rule |
|---|---|
| Which messages | Only messages parked with `x-nibras-failure-kind: transient`; never a permanent failure and never a `.dlq` crash loop, which needs a new image first (section 4.3) |
| Which queues | Never a queue that reaches a parent. Every queue named in the "Messages that reach a parent" row of section 4.5 is excluded: every `notification-worker` queue, every `notification.events` queue, `notification.commands`, and any queue whose handler sends `RequestNotification`, as `.claude/rules/messaging.md` requires. A message parked on one of them is replayed only from the console, as a human decision with a recorded reason; the exclusion list is that row, not a copy, so a queue added to it is excluded here the same day |
| When | Only after the dependency named by the last error reports healthy again, as Platform's job checks every 5 minutes |
| How often | At most three automatic replays per message (the Platform sheet's bound); after the third the message stays parked for an operator |
| How | Through the same `ReplayParkedMessages` command and execution rules as a console replay (section 4.5): throttled, order kept behind a blocked key, the policy recorded as the actor and "transient cause cleared" as the reason in `<service>.audit.recorded.v1`, and a note to the operator on the console |
...
Master brief Section 8 item 12: consumer tests with Testcontainers, contract tests for message schemas, and a chaos test that kills a worker mid-batch and proves nothing is lost or duplicated. `16-test-strategy.md` owns the strategy and the coverage matrix; these are the messaging suites it schedules. Every integration suite runs RabbitMQ 4 with the consistent-hash plug-in and PostgreSQL in Testcontainers on the Linux runner (`ubuntu-latest`); the suites of `Nibras.BuildingBlocks.Messaging` (outbox, inbox, dispatcher, retry and parking) also run on `windows-latest`, the Windows integration run document 33 part 4 keeps for the building blocks (section 14).

Each suite carries one test-case identifier from this document's block `TC-MSG-750` to `TC-MSG-759`, defined here and cited elsewhere as "`TC-MSG-75N` (document 11)" (ADR-0020). Where one identifier covers two rows, the rows are two assertions of one guarantee.

| Test case | Suite | What it proves | Class and location |
|---|---|---|---|
...
| TC-MSG-751 | Dispatcher leader killed | The leader is killed between publish and confirm; the standby takes the lock within 10 s; every row is published, some twice, and every consumer applies each once | `MessagingChaosTests.DispatcherLeaderKilled_StandbyResumes_NoLoss` |
| TC-MSG-751 | Broker node lost | In the three-node scale topology a node is stopped mid-batch; quorum queues keep serving; nothing is lost | `MessagingChaosTests.BrokerNodeLost_QuorumQueuesContinue`, in the phase 6 chaos drill |
| TC-MSG-752 | **Deliver twice** | Every handler receives the same message twice with one `messageId` (inbox path) and twice with two `messageId` values (natural-key path, as a dual-version publish or a replay would produce); the effect happens once either way | `IdempotentConsumerTestBase<THandler, TMessage>`, one derived class per handler; `MessagingConventions.EveryHandlerHasDeliverTwiceTest` fails a handler without one |
| TC-MSG-753 | **Ordering** | 10,000 events over 200 keys through the 8 projection shards, with a replica killed and another added mid-run, arrive in commit order per key | `OrderingTests.SameKey_ProcessedInCommitOrder_AcrossRebalance` |
| TC-MSG-753 | Ordering, blocked key | A poisoned message blocks its key; the other keys on the shard keep flowing; replaying it releases the key in order | `OrderingTests.FailedMessage_BlocksKey_OthersFlow` |
| TC-MSG-753 | Ordering, outbox horizon | Two transactions insert rows for one key and commit in the reverse order of their sequence; the dispatcher still publishes in sequence order | `OutboxDispatcherTests.ConcurrentTransactions_PublishedInSequenceOrder` |
| TC-MSG-754 | Unordered queues | Messages delivered in a shuffled order leave the same final state | `ReferenceCopyTests.ShuffledDelivery_SameFinalState` per consuming service |
| TC-MSG-755 | Retry and parking | Transient failures follow the queue's profile and park; permanent failures park at once; three crashes land in `.dlq` | `RetryPolicyTests` in `Nibras.BuildingBlocks.Messaging.Tests` |
| TC-MSG-755 | Replay console | Replay resets the attempt, records the reason and the audit event; a message parked behind another cannot be replayed alone; a parent-facing replay needs the extra confirmation | `ParkingLotReplayTests`; the replay-applies-once integration test behind the failed-messages screen is `TC-MSG-901` (Platform sheet), which `08-web-structure.md` cites for that screen |
| TC-MSG-756 | Lanes | With 100,000 digest messages queued on the bulk lane, an absence alert is dispatched within 30 s of being enqueued. The 30 s is the lane's own budget; when Attendance enqueues the alert (at the mark, or after the register closes) is Open Question 27, still open (RISK-41), and does not change this test | `LaneIsolationTests.BulkBacklog_UrgentStillWithin30Seconds` |
| TC-MSG-757 | Tenant fairness | Tenant A publishes 3,000 report-card renders; tenant B publishes 40 one second later; B's batch completes within 2 minutes while A's is still running | `TenantFairnessTests.LargeTenantBatch_SmallTenantNotStarved` |
| TC-MSG-758 | Outbox | A rolled-back transaction leaves no outbox row; a bulk update without explicit enqueue fails the architecture test | `OutboxTests`, `MessagingConventions.BulkUpdateOnPublishingAggregateEnqueuesExplicitly` |
| TC-MSG-758 | Topology | Declared queues, arguments and bindings equal the section 2 tables | `MessagingConventions.EveryQueueMatchesDocument11` (section 1.5) |
| TC-MSG-758 | Command senders bound | Every service whose code sends a command (`RequestNotification`, `GenerateDocument` and the rest of section 2.4) publishes on an exchange that the target's `<service>.commands` queue binds; a sender added without its binding fails the build instead of parking messages on no queue | `MessagingConventions.EveryCommandSenderIsBound` |
| TC-MSG-759 | **PactNet contracts** | Each consumer's contract suite records, per consumed key, the message it expects (routing key, envelope headers, payload fields and types) as a PactNet message pact; the provider's suite verifies every pact against its real mapper from domain event to contract. Commands and replies are covered the same way between orchestrator and target. A provider change that breaks a pact fails the provider's pipeline, which is how section 3's rule "breaking means a new version" is enforced before a merge. The provider suite also serializes every contract once under the process cultures `ar-SA`, `en-US` and `de-DE` and asserts byte-identical JSON: instants in ISO 8601 UTC, amounts with a `.` decimal point and Latin digits, never the Um Al Qura calendar that `ar-SA` formats by default | `Nibras.<Consumer>.ContractTests/Messaging/` and `Nibras.<Provider>.ContractTests/Messaging/` (`07-solution-structure.md`); pact files are pipeline artifacts under `contracts/pacts/` |

### 13. Risks this document carries

`18-risk-register.md` owns the register, the scoring scale and the identifiers; this table is the messaging view of it and adds no identifier of its own. Likelihood and impact use the 1 to 5 scale of that document's part 1. A row whose "In the register" cell names no identifier scores below 12, the register's own threshold (ADR-0022), and stays local to this document: its owner re-scores it at each phase gate and enters it in document 18 if the score reaches 12.

| Risk | L | I | Mitigation in this document | Owner role | In the register |
|---|---|---|---|---|---|
| A message is lost between the state change and the broker, or published twice, so a guardian is charged twice or never told | 2 | 5 | The outbox of section 8, written in the same transaction as the state change, with the dispatcher's advisory lock of section 8.3 and the inbox deduplication of section 8.1; the deliver-twice suite `TC-MSG-752` and the worker-kill chaos case `TC-MSG-750` of section 12 | Architect | none; score 10, local. If Wolverine is the cause, the library risk is RISK-12 |
| A large tenant's bulk work starves a small tenant, or bulk backlog delays an urgent message | 3 | 4 | The three lanes and their separate queues (section 1.3), the per-tenant admission window, in-flight cap and send rate of section 5, and the prefetch limits per queue in section 2.5; `LaneIsolationTests` (`TC-MSG-756`) and `TenantFairnessTests` (`TC-MSG-757`) in section 12, with Appendix N scenarios N-06 and N-09 in phase 6 | Architect | RISK-19 |
| One dispatcher leader per service cannot hold Notification's peak, so `nibras_outbox_oldest_pending_seconds` climbs during the 08:00 window | 3 | 3 | Open point 5 states the default and the measured trigger; the named fallback is N leases by hash of `partition_key`, each with its own advisory lock, which keeps per-key order because a key maps to one lease; the metric and its alert are in section 11 | Performance engineer | none; score 9, local, and tracked through open point 5 |
| Wolverine proves unsuitable at load and the messaging library has to change mid-delivery | 2 | 4 | ADR-0004 and open point 2 keep the outbox, inbox, saga tables and dispatcher inside `Nibras.BuildingBlocks.Messaging` with Wolverine's durable storage off, so the contract of sections 8 and 9 survives the swap; the section 12 suites are written against the building block, not the library | Architect | RISK-12 |
| A parked message that reaches a parent is replayed automatically or without thought, and a stale absence alert frightens a family | 2 | 4 | Nothing that reaches a parent is replayed automatically, and the written replay policy of section 4.4 excludes those queues (Decisions in force); the parking lot of section 4.5 preselects messages older than their relevance window for discard and requires the operator's explicit confirmation before a parent-facing replay is enabled, with the reason recorded in the audit event; `TC-MSG-755` | Product owner | none; score 8, local |

### 14. Platform notes and signature features

| Concern | What holds here | Proof and runner |
|---|---|---|
| Runner | RabbitMQ and PostgreSQL run as Linux containers, so every messaging suite of section 12 runs on `ubuntu-latest`; `Nibras.BuildingBlocks.Messaging` also runs on `windows-latest`, as document 33 part 4 keeps for the building blocks. The chaos cases `TC-MSG-750` and `TC-MSG-751` run on the Linux runner and, for the broker-node case, in the phase 6 chaos drill on the scale topology. `dev-smoke` starts the broker with every consumer on `ubuntu-latest`, `windows-latest` and `macos-latest` | Document 33 part 4 |
| Culture-sensitive values | Every envelope field and payload is machine-facing: instants in ISO 8601 UTC, amounts and counts under the invariant culture, identifiers as strings, so a message published by an `ar-SA` process reads the same in an `en-US` one; routing keys and queue names are ASCII and compared ordinally | `TC-MSG-759` (the three-culture serialization case); `TC-PLAT-007` (document 33) for money parsing |
| Right-to-left and bilingual text | Messages carry identifiers and codes, not sentences; where Appendix E carries text it travels as stored and the recipient renders it. Direction is decided where the text is shown: Notification's templates and the clients (`24-localization-and-calendars.md`) | `TC-MSG-759` pins every payload shape |
| Devices without Google services | They matter to the urgent lane: an absence alert or emergency broadcast for such a device is delivered in-app while the app is open and falls back to SMS and email (REQ-NOT-019). The fallback is a new channel command on the same urgent lane (section 2.5, Notification), so it never queues behind bulk work | `TC-NOT-610` (Notification sheet); `TC-MSG-756` for the lane |

**Signature features.** This document owns none. Its lanes, ordering and outbox are what features 3 (sixty-second attendance) and 32 (emergency mode) need on the urgent lane, what 6 (Report Card Studio) and 9 (import with dry run and rollback) need on the bulk lanes and worker sagas, what 7 (the parent's one calm message) needs from Notification's dispatch stage, and what 40 (self-healing operations) needs from the parking lot and replay policy of section 4, cited by their Appendix W numbers. Each feature's moment, rung, autonomy, the requirements and slices that build it, its Appendix O step and its demo test are held once, in the "Signature feature trace" table of `32-product-differentiation-and-demo.md`, and are not copied here.
...
| Nothing that reaches a parent is replayed automatically; other queues replay a transient failure automatically only inside the written replay policy of section 4.4 | `.claude/rules/messaging.md`; master brief Section 12.1 item 40; REQ-PLT-037 | As stated | Without the bounds an automatic replay is an unbounded retry profile; without the exclusion a stale alert reaches a family |
...
|---|---|---|---|---|---|---|---|
| 1. Should Appendix E's consumer columns gain the bindings of section 2.6 (reference copies from doc 10, saga outcomes from doc 13)? | Yes: the bindings are declared now, and one brief amendment with an ADR and a version bump on all three briefs adds the consumers to Appendix E | Product owner, through the architect | If the amendment is refused, each starred binding needs another source: a reply beside the event for saga outcomes, and a gRPC snapshot for the reference copies | 2 | 3 | 6 | RISK-15 |
| 2. Map Wolverine's durable message store onto these tables instead of owning the dispatcher? | No: the building block owns the dispatcher and Wolverine's durable storage is off | Tech lead, with ADR-0004 | If mapped instead, section 8.3 is replaced by Wolverine's agent and the tables keep their shape; the tests of section 12 do not change | 2 | 2 | 4 | RISK-12 |
| 3. How long are terminal `saga_instances` rows kept? | 13 months, then soft-deleted by a `SagaRetentionJob` and purged 30 days later; `Deleted` and `Sealed` sagas follow the Appendix J period of the record they concern | Data architect, in `10-data-architecture.md` section 8 | A longer period costs only storage; a shorter one loses the process monitor's history for the year-end rollover | 2 | 1 | 2 | none |
| 4. Record in doc 10 section 4 that the outbox and inbox carry no audit or soft-delete columns | Exempt, as section 8.1 explains | Data architect | If refused, the two tables gain six unused columns and the persistence generator needs no exception | 1 | 1 | 1 | none |
| 5. Is one dispatcher leader per service enough for Notification at peak? | Yes, until a load measurement shows `nibras_outbox_oldest_pending_seconds` above 5 s at the Appendix N peak. The first measurement is proposed for phase 1, not phase 6: a spike that drives one leader at the N-09 fan-out rate against the foundation outbox as soon as SL-MSG-001 exists, so that a split into leases, if needed, lands before twenty services depend on the dispatcher. Document 34 schedules it as the exit check of SL-MSG-001 in phase 1, read against the 5-second line | Performance engineer | If not, dispatch is split into N leases by hash of `partition_key`, each with its own advisory lock; per-key order is kept because a key maps to one lease | 3 | 3 | 9 | RISK-12, RISK-19 |
| 6. Should section 1.3's list of urgent traffic be aligned with Appendix C for dismissal, vehicle delays and scan failures? | Appendix C decides the inbound lane (section 2.5, Notification) | Product owner | If those three become urgent in Appendix C, three keys move from `notification.events` to `notification.events.urgent`; nothing else changes | 3 | 2 | 6 | none |
| 7. Does Ai consume the change events of indexed sources, as `05-service-catalog.md` says? | Yes. Closed 2026-09-22 by ADR-0019: Appendix E names Ai as a consumer of `communication.announcement.published.v1`, `assessment.report-cards.published.v1`, `academics.lesson-plan.submitted.v1`, `attendance.attendance.marked.v1`, `behavior.incident.recorded.v1` and `school.student.section-changed.v1`, and section 2.5 binds all six on `ai.index-sources.bulk`; the record is still read through the Bff.Web source feed | Architect | None left: the default is the catalog. If a change key is missed, `IndexSyncJob` catches up within 15 minutes and the nightly `IndexReconciliationJob` proves the index (`06-services/ai.md` §10) | 1 | 1 | 1 | none |
...
|---|---|---|
| Every key in this document exists in Appendix E, except the command, reply and worker-job keys of section 2.4, whose names the section 2.4 command catalog lists | Kit-lint R27: every backticked routing key here is in Appendix E, or is a `commands` or `replies` key whose PascalCase name this document writes in backticks | Lint (`/lint-plan`) on every change under `docs/` |
| Every Appendix E consumer entry has a binding in section 2.5 | Review step, not a lint rule: the `messaging-reviewer` agent (`/audit-messaging`) compares every consumer entry of the Appendix E tables with the section 2.5 queue tables, and every binding with an Appendix E consumer, at the Group C review and on every change to this document or Appendix E | Group C review; every change to this document |
| Every queue name follows section 1.2 | Review step, not a lint rule: the `messaging-reviewer` agent checks every queue name of section 2.5 against the section 1.2 pattern at the Group C review and on every change to this document. Once code exists, the product build adds `MessagingConventions.EveryQueueMatchesDocument11` to `tests/Architecture.Tests` (document 07 part 4), over the queue declarations SL-MSG-002 builds | Group C review; `tests/Architecture.Tests` from SL-MSG-002 |
| No routing key is invented or redefined | Kit-lint R27 for this document (a key must be in Appendix E or be a section 2.4 command or reply this document names) and R19 for every other plan document, the service sheets included (a key must be in Appendix E or this document). That a catalogued key keeps its Appendix E meaning, publisher and payload is a review step for the `messaging-reviewer` agent at the Group C review | Lint (`/lint-plan`); Group C review |
| Every worker named is one of the seven in Appendix L | Kit-lint R31 checks every backticked service-led image name (`nibras/<service>-<kind>`) and database name in the plan against Appendix L. The worker host names (`<Service>.Worker`, `Reporting.Projections`) are not linted: the `messaging-reviewer` agent compares them with the seven Appendix L workers at the Group C review and on every change to this document | Lint (`/lint-plan`); Group C review |
| Every fenced SQL column has a comment and every Mermaid block has a known type | Kit-lint R30 (every column of a `CREATE TABLE` has a comment) and R17 (every Mermaid block opens with a known type); the plan-document review checklist for any SQL outside a `CREATE TABLE` | Lint, review |
| Nothing is lost or duplicated when a worker, the dispatcher leader or a broker node dies | `TC-MSG-750` and `TC-MSG-751`, the chaos suites of section 12, built by SL-MSG-006 over the outbox of SL-MSG-001 and the inbox of SL-MSG-002 | Service integration suites on `ubuntu-latest`; the phase 6 chaos drill |
| Every handler is idempotent | `TC-MSG-752`, the deliver-twice suite, and its architecture test `MessagingConventions.EveryHandlerHasDeliverTwiceTest` | Service integration suites; `tests/Architecture.Tests` |
| Order holds per partition key on the ordered queues, and unordered queues converge | `TC-MSG-753` (three ordering cases) and `TC-MSG-754` | `Reporting.Projections` and Audit integration suites; each consuming service |
| A failure retries on its profile, parks, and replays only by an operator or inside the written replay policy | `TC-MSG-755`; `TC-PLT-122` (Platform sheet) for the automatic half | `Nibras.BuildingBlocks.Messaging.Tests` on `ubuntu-latest` and `windows-latest` |
| An urgent message is never delayed by bulk work | `TC-MSG-756`; `UrgentEndToEndSlow` in production | Integration suite; alerting |
| One tenant cannot starve another | `TC-MSG-757`; Appendix N load scenarios in phase 6 | Integration suite; `tests/Load/` |
| The outbox and the declared topology match this document | `TC-MSG-758`, built with SL-MSG-001 and SL-MSG-002 | Integration suite; `tests/Architecture.Tests` |
| Every message contract change is caught before merge, and no payload depends on the host culture | `TC-MSG-759`, PactNet provider verification with the three-culture serialization case | Each provider's pipeline |
