...
|---|---|---|---|---|---|
| A message is lost between the state change and the broker, or published twice, so a guardian is charged twice or never told | 2 | 5 | The outbox of section 8, written in the same transaction as the state change, with the dispatcher's advisory lock of section 8.3 and the inbox deduplication of section 8.1; the deliver-twice suite and the worker-kill chaos case of section 12 | Architect | none yet; Architect proposes it at the Group E review |
| A large tenant's bulk work starves a small tenant, or bulk backlog delays an urgent message | 3 | 4 | The three lanes and their separate queues (section 1.3), the per-tenant admission window, in-flight cap and send rate of section 5, and the prefetch limits per queue in section 2.5; `LaneIsolationTests` and `TenantFairnessTests` in section 12, with Appendix N scenarios N-06 and N-09 in phase 6 | Architect | RISK-19 |
| One dispatcher leader per service cannot hold Notification's peak, so `nibras_outbox_oldest_pending_seconds` climbs during the 08:00 window | 3 | 3 | Open point 5 states the default and the measured trigger; the named fallback is N leases by hash of `partition_key`, each with its own advisory lock, which keeps per-key order because a key maps to one lease; the metric and its alert are in section 11 | Performance engineer | none yet; Performance engineer proposes it at the Group E review |
...
| A parked message that reaches a parent is replayed automatically or without thought, and a stale absence alert frightens a family | 2 | 4 | Nothing is replayed automatically on any queue (Decisions in force); the parking lot of section 4.5 preselects messages older than their relevance window for discard and requires the operator's explicit confirmation before a parent-facing replay is enabled, with the reason recorded in the audit event | Product owner | none yet; Architect proposes it at the Group E review |
