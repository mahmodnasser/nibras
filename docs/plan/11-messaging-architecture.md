...
| Notification | `RequestNotification` | `nibras.platform`, `nibras.admissions`, `nibras.school`, `nibras.finance`, `nibras.requests`, `nibras.academics`, `nibras.scheduling`, `nibras.identity`, `nibras.ai`, `nibras.audit` | 1, 2, 3, 5, 6, 8; and the Appendix E jobs that publish `notification.notification.requested.v1`: assignment due reminder (Academics), uncovered-period escalation (Scheduling), joining timeouts (Identity), deletion cooling-off reminder, API key expiry reminder, sandbox lifecycle, support SLA and subject request deadline (Platform), re-enrollment window (Admissions), daily digest of blocked injection attempts (Ai, Appendix C "AI injection attempts blocked"), and the `AuditExportJob` notice to the principal, `06-services/audit.md` section 9 (the job) and section 10.2 (T-AUD-03) | `notification.notification.requested.v1`, then `notification.notification.delivered.v1` |
...
| `notification.commands` | also on `nibras.admissions`, `nibras.school`, `nibras.finance`, `nibras.requests`, `nibras.academics`, `nibras.scheduling`, `nibras.identity`, `nibras.ai`, `nibras.audit` (`RequestNotification`, section 2.4) | standard | `sagaId`, or `userId` for a job's request | 16 | C 3 | standard |
...
| `behavior.events` | `documents.document.generated.v1` (the award certificate and portfolio book ids Behavior stores) | standard | `subjectId` | 16 | S 4 | standard |
