...
| **1 Foundation** | Everything every later service stands on | Gateway, Bff.Web, Identity, Platform, Notification, Audit, and the Documents rendering pipeline | 14 to 22 weeks | 19 | 14 | 153 plus the cross-cutting obligations | A tenant is provisioned live, its administrator signs in with a second factor, a notification arrives, the audit entry is visible; minute 15 of Appendix O (a report-card template started from the template exchange, Appendix W feature 37) and reserve step R-18 as scripted, the steps whose Phase cell is 1 |
...
| **3 Money and paperwork** | A fee is invoiced, chased and paid; a request is approved and takes effect; a certificate verifies | Finance, Requests, Communication, Documents | 11 to 18 weeks | 14 | 9 | 103 | Minute 10 of Appendix O now as scripted, so act two runs as written, with every minute the phase 2 demo ran as scripted; minutes 1, 5, 12 and 14 keep their substitutions (TC-RQS-002; TC-RPT-008 against the threshold rule; TC-SEC-530; reserve step R-01, TC-L10N-311) |
| **4 Growth** | An applicant becomes a student; dashboards answer Appendix D; mobile reaches parity | Admissions, Behavior, Reporting | 7 to 12 weeks | 9 | 3 | 61 | Fourteen of the fifteen Appendix O minutes as scripted on a fresh tenant with one-click reset; minute 12 keeps its substitution (TC-SEC-530), because its Phase cell is 5 |
...
| Estimate sensitivity | Phase 3: the ZATCA plug-in, at about twice its estimate | The ZATCA slices SL-FIN-448 to SL-FIN-450 (9 slice-days, conditional under open question 9's default for a VAT-registered first customer in Saudi Arabia) are the one estimate of low confidence, RISK-53 in `18-risk-register.md`; document 34 part C puts a reasonable upper bound at about twice. At that bound phase 3 carries 344 slice-days, not 335, and `schedule-34.mjs` gives 12 to 18 weeks instead of 11 to 18 (its dependency floor rises to 8 weeks, still under the effort figure) and 62 to 95 weeks to launch instead of 61 to 95; the MVP figure is unchanged. The ranges above keep the estimate; an overrun is RISK-53's signal and is folded in when the slices are re-estimated |

These replace the ranges of master brief Section 28, which were set before any slice existed and put phase 1 at 8 to 10 weeks against 423 slice-days. The brief is corrected to match under ADR-0019. A team outside Section 29's shape changes the builders input and nothing else; the ranges are recomputed, never adjusted by hand. For example, a team whose quality engineer takes no slices has four to seven builders, and the same formula over the same slices gives phase 1 16 to 28 weeks and launch 68 to 116 weeks; that difference is the capacity the quality engineer's slices carry.

**Phases 1 and 2 are longer than `PLAN_SPEC.md` sizes a phase.** PLAN_SPEC sets a phase at 6 to 20 weeks; phase 1 reaches 22 weeks and phase 2 21 weeks at the five-builder end, and both sit inside it only with eight builders. The overrun is stated rather than trimmed, because the ranges are derived from the slices: phase 1 holds the building blocks, the pipeline and the six services every later phase stands on, and phase 2 the six services of the school year loop up to the MVP cut line, so a shorter figure would hide effort, not remove it. The alternative is to split each phase in two at a capability boundary of about equal slice-days (phase 1 after the building blocks and Identity, phase 2 after School and Scheduling), which changes no slice and no total and adds one demonstration to each; the product owner chooses between the two, and until then the phases stand as listed. The overrun is carried under RISK-03 in `18-risk-register.md`, and document 34 Section 2 says the same.
...
| CAP-FIN-06 | The cashier closes the day balanced, and the books export cleanly | Finance | WF-FIN-06 | 2 to 3, with the conditional e-invoicing plug-ins of document 34 (SL-FIN-448 to SL-FIN-452) |
...
| CAP-INT-02 | A teacher launches a registered external learning tool from a section, an assignment or a resource, and the tool reads only the roster its privacy setting allows | Platform, Academics, Assessment | none | 1 to 2 |

**Exit criteria.** Every Appendix O minute whose Phase cell is 4 or earlier passes as scripted on a freshly provisioned tenant with one-click reset, which is fourteen of the fifteen; minute 12 passes by its substitution until phase 5; the principal-dashboard scenario from Appendix N passes; an LTI 1.3 launch with deep linking and names and roles passes on the demo tenant, from a section and from an assignment or resource.
...
CAP-INT-02 and CAP-INT-03 carry the REQ-INT-016 standards work that the Platform service sheet and the requirement itself time to phases 4 and 5. `34-work-breakdown.md` lists SL-INT-411 and SL-ACA-405 under CAP-INT-02 and SL-INT-600 to SL-INT-602 under CAP-INT-03. Moving SL-INT-411 and SL-INT-600 to SL-INT-602 there from CAP-ACA-03 and CAP-AI-01 changed no days, services or dependencies. SL-ACA-405 is the Academics launch from an assignment or resource (REQ-ACA-030), taken out of the phase 2 slice SL-ACA-207 because a launch cannot run before SL-INT-411's registration, login and keys; it adds Academics to CAP-INT-02's services and two slice-days to phase 4, which the Section 1 ranges include. LTI Assignment and Grade Services 2.0, in scope in `23-integrations-and-public-api.md` §6.3, is built by SL-INT-412 (Platform: the tool's grade-scoped token, line items bound to the SL-ACA-405 launch, the score and result services) and SL-ASM-400 (Assessment: the posted score lands as a mark in the entry state, which the teacher approves by submitting the component, never published by the tool), proved by `TC-INT-036`. They add Assessment to CAP-INT-02's services and three slice-days to phase 4 (212 to 215), and `schedule-34.mjs` gives the same phase 4 range of 7 to 12 weeks, because before rounding up the low end moves from 6.89 to 6.99 working weeks and the high end from 11.02 to 11.18. One more phase 4 slice-day would take the low end to 8 weeks.
...
- **Move.** No reason stands against the brief. ADR-0024 proposes moving it, and the change is logged for document 34; until the product owner decides, document 34 still builds it where the row says. REQ-ACA-030 was the one Move row that could not wait, because its launch cannot run in phase 2; document 34 has moved it to phase 4, and its row now gives the reason it is built there.
...
| REQ-ACA-030 | 2 | SL-ACA-405, SL-INT-412, SL-ASM-400 | 4 | Shared, after a move. An LTI 1.3 launch needs the tool registration, third-party login and signing keys that SL-INT-411 builds in phase 4 for the Tier 1 REQ-INT-016, so the launch was moved out of phase 2 rather than waiting on ADR-0024: SL-ACA-405 (`LaunchLtiTool`, `POST /lti-launches`) builds it under CAP-INT-02 beside SL-INT-411, and in phase 2 SL-ACA-207 builds the Tier 1 resource library (REQ-ACA-022) only. The launch from an assignment or resource is the same launch REQ-INT-016's phase 4 checklist runs, started from Academics records, so deferring it further would reopen CAP-INT-02 in phase 5. No phase 2 or 3 demo or exit criterion claims a launch, and the phase 4 exit runs the first |
...
**What deferral would save.** The slices above that build nothing but Tier 2 or Tier 3 work come to 59 slice-days in phases 1 to 4 (5 in phase 1, 24 in phase 2, 12 in phase 3, 18 in phase 4, SL-ACA-405 among them), 37 of them inside the MVP of Section 5, computed from document 34. The rows marked Move are the part ADR-0024 proposes to take out; the Locality rows are the next candidates if the MVP must come sooner.
...
| Team size and shape | Master brief Section 29, five to eight builders. Every phase range in Section 1 assumes it: phase 1 is 14 weeks with eight builders and 22 with five. By the same formula a team of four stretches phases 1 and 2 by about six weeks each over the five-builder figure and phases 3 to 5 by two to four weeks, while phase 6 stays inside its calendar floor. A different team changes the builders input of `schedule-34.mjs` and the ranges are recomputed. Scored as `01-questions-and-assumptions.md`, which owns Open Question 24, scores it | Product owner, Open Question 24 | 3 | 3 | 9 | RISK-06, RISK-35 |
| Whether the product owner accepts ADR-0024: the 35 requirements built ahead of their tier, the Move rows taken out of phases 1 to 4, and the Locality rows kept | Built as listed; document 34 is unchanged until the decision, and the Move rows are logged for it, except REQ-ACA-030, whose launch document 34 already builds in phase 4 as SL-ACA-405 beside SL-INT-411 because it cannot run earlier. If the default is wrong, phases 1 to 4 carry up to 59 slice-days of Tier 2 and Tier 3 work, 37 of them inside the MVP | Product owner, with the architect | 4 | 3 | 12 | RISK-02 |
...
| 2026-09-26 | Round-4 scorecard, Group F, then remediation round 5 | Group F was blocked on consistency by SL-ACA-207 building `LaunchLtiTool` in phase 2 against this document; Group E was approved with minor gaps. Amended: SL-ACA-207 builds the resource library only, and REQ-ACA-030's launch is SL-ACA-405 under CAP-INT-02 (services Platform, Academics) in phase 4, its row restated with the reason and "Logged for document 34" removed; the country e-invoicing plug-ins estimated in document 34 as conditional slices SL-FIN-448 to SL-FIN-452 (14 slice-days in phase 3); ranges recomputed by `schedule-34.mjs`: phase 3 from 11 to 17 to 11 to 18 weeks, phase 4 from 7 to 11 to 7 to 12, launch from 61 to 93 to 61 to 95 weeks (about 71 to 110 calendar weeks), the MVP unchanged at 42 capabilities, 954 slice-days and 33 to 50 weeks; Tier 2 and 3 slice-days in phases 1 to 4 from 57 to 59; the phase 1 and 2 overrun of the PLAN_SPEC phase size stated with the split alternative; minute 15 named in the phase 1 demo cell; the team-size open point scored 3 × 3 = 9 against RISK-06 and RISK-35, as document 01 scores Open Question 24 |
| 2026-09-26 | Round-5 scorecard, remediation round 6 | Groups E and F were approved with minor gaps. Amended: CAP-INT-02 gains SL-INT-412 and SL-ASM-400 for LTI Assignment and Grade Services (Assessment added to its services, weeks 1 to 2), and REQ-ACA-030's row names them; `schedule-34.mjs` over the regenerated document 34 gives unchanged ranges (phase 4 215 slice-days, 7 to 12 weeks; launch 61 to 95; MVP 42 capabilities, 954 slice-days, 33 to 50 weeks). Checked and already closed: minute 15 is named in the phase 1 demo cell |
| 2026-09-26 | Round-6 scorecard, remediation round 7 | Group F was approved with minor gaps. Amended: Section 1's input table gains the phase 3 sensitivity to the ZATCA upper bound, computed with `schedule-34.mjs` on document 34 with the three ZATCA slices doubled (phase 3 12 to 18 weeks, launch 62 to 95); the stated ranges are unchanged |
...
| Every capability is broken into slices | kit-lint R25 fails when a `CAP-` row here has no `SL-` row under its `#### CAP-` heading in document 34. CAP-INT-02 holds SL-INT-411, SL-ACA-405, SL-INT-412 and SL-ASM-400, and CAP-INT-03 holds SL-INT-600 to SL-INT-602 |
