...
| Assignment and Grade Services 2.0: the tool posts scores that land as marks awaiting teacher approval, never directly published; built by SL-INT-412 (Platform: grade-scoped token, line items, scores, results) and SL-ASM-400 (Assessment: the mark in the entry state) in phase 4, proved by `TC-INT-036` | A tool reading marks it did not post |
...
| 7. Open question 9: which regional plug-ins come first? §8 ships the interfaces in Tier 1 and leaves each country's implementation to the plug-in's own repository. For e-invoicing the question is still open, not settled | The default of open question 9 in `OPEN_QUESTIONS.md` stays in force: interfaces in Tier 1 and implementations per country in phase 5, except the e-invoicing plug-in for the country of a VAT-registered first customer, which the Finance owner's team builds in phase 3. `34-work-breakdown.md` SL-FIN-444 builds only the e-invoicing hand-off and the outage queue in phase 3, against the plug-in contract and tested with the fake provider; it submits nothing to an authority. **Who builds the plug-ins:** the Finance owner's team, as the conditional phase 3 slices that follow SL-FIN-444 in `34-work-breakdown.md`: SL-FIN-448 to SL-FIN-450 for ZATCA in Saudi Arabia (9 slice-days, certified by `TC-FIN-620`) and SL-FIN-451 and SL-FIN-452 for JoFotara in Jordan (5 slice-days, certified by `TC-INT-037`), unless a certified partner builds one against §8.3 first. Both sets are counted in the phase 3 range of `17-roadmap.md`, and each is conditional on open question 3 (a VAT-registered first customer in its country), as open question 9's default states: a set whose country has no such customer, or all of them if the product owner overturns that default, moves to phase 5 unchanged, and phase 3 is 9 or 5 slice-days lighter. **Decision point:** the Phase 2 exit review, where the product owner decides open question 9 for e-invoicing and the plug-in joins the phase 3 slice list or SL-FIN-444 ships with the accounting export only (RISK-53's signal: the question still open at the Phase 2 exit). Until then Finance runs with the accounting export and the queued submission path of SL-FIN-444 | Product owner decides at the Phase 2 exit review; the Finance owner builds the first plug-in | A Saudi or Jordanian school that runs its fees on Nibras from phase 3 cannot submit its tax invoices from it and keeps a second system for e-invoicing until phase 5, which qualifies the Finance promise at its launch | 4 | 3 | 12 | RISK-53 |
...
| 2026-09-26 | Round-5 scorecard, remediation round 6 | Amended; awaiting the round 6 score | Group F was approved with minor gaps. §6.3 names the slices that build Assignment and Grade Services, SL-INT-412 and SL-ASM-400 in phase 4, and `TC-INT-036` proves the score lands as a mark awaiting the teacher; `TC-INT-037` certifies the JoFotara plug-in; open point 7 restated from the current SL-FIN-444 (hand-off and queue only) and the both-countries count in phase 3, each set conditional on open questions 3 and 9 |
| 2026-09-26 | Round-6 scorecard, remediation round 7 | Amended; awaiting the round 7 score | Group F was approved with minor gaps. Open point 7 states open question 9's default as `OPEN_QUESTIONS.md` does: the e-invoicing plug-in for a VAT-registered first customer's country is built in phase 3 by default, and the other country implementations in phase 5 |
...
| Standards | `TC-INT-030` iCal feed validates against RFC 5545 and updates by `SEQUENCE`; `TC-INT-031` OneRoster 1.2 responses validate against the published schemas and CSV binding; `TC-INT-032` an LTI 1.3 launch and a score post with the standard body's reference tool; `TC-INT-036` the posted score lands in Assessment as a mark awaiting the teacher, never published, and the grade scopes refuse what they do not allow; `TC-INT-033` QTI 3 round trip of each in-scope interaction; `TC-INT-034` an Open Badges 3.0 credential verifies with an independent verifier; `TC-INT-035` CASE import and re-import | Nightly from the phase that ships each standard |
...
| The plug-in kit | `TC-INT-002`: the sample plug-in passes the conformance suite on Windows and Linux under three cultures; every fake implementation backs a sandbox end-to-end run; each country e-invoicing plug-in is certified by its own case, `TC-FIN-620` (Finance sheet) for ZATCA and `TC-INT-037` for JoFotara | Every pull request touching `Nibras.Plugins.*` |
...
| TC-INT-036 | Given a tool registered with grade access and launched by a teacher from a grade 4 science assignment, when it obtains a token by the client-credentials grant, creates a line item and posts a score for one student twice with the same timestamp, then one line item is bound to that assignment, Assessment holds exactly one mark for the student in the entry state with the tool as its source and nothing is published until the teacher submits the component; and a token without the score scope, a score for a student the tool's privacy setting hides, a score for a locked component and a read of results the tool did not post are each refused | REQ-INT-016, REQ-ACA-030 |
| TC-INT-037 | Given the JoFotara e-invoicing plug-in version under certification and the national system's sandbox, when the §8.3 checklist runs, then the conformance suite passes for submission, status polling and credit notes, an invoice submitted twice returns the stored reference and QR, a rejection raises a task carrying the authority's reason, and each posted invoice of the test month is submitted once and appears in that month's export | REQ-FIN-032, REQ-INT-018, REQ-INT-019 |
