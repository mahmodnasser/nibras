...
| Rule | Detail |
|---|---|
| What can be subscribed | Only routing keys in the eligibility table below; the table is generated into `docs/api/webhook-events.json`, and a check in `tools/api-lint/` fails when a key in it is absent from Appendix E |
...
| 5. Webhook quota breach skips deliveries rather than delaying them | Skip and allow replay after reset | Product owner | Delaying keeps order but grows the queue without bound on a misconfigured integration |
...
| Only eligible events are subscribable and every one exists in Appendix E | `TC-INT-016`; the `webhook-events.json` check in `tools/api-lint/` | Every pull request touching Appendix E or this document |
