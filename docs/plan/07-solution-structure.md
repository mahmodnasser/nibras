...
**Who builds it, and over how long, quoted from `17-roadmap.md`.** The team is master brief Section 29's, "five to eight builders", and every phase range in document 17 Section 1 assumes it: phase 1, which creates most of this structure, is 14 to 22 weeks; phase 2 14 to 21; phase 3 11 to 18; phase 4 7 to 12; phase 5 9 to 14; phase 6 6 to 8; and "61 to 95 weeks" from the start of phase 1 to launch. The structure is sized for that team because the template generates each service's projects (part 3), so no builder writes the 206 project files by hand.
...
│   ├── Rules/                                  one class per rule group: Layers, BuildingBlocks, Contracts, Services, Handlers, Persistence, Hosts, Caching, Messaging, Portability, and GrpcHopRules
...
All in `tests/Architecture.Tests/Rules/`, written with NetArchTest against every assembly the fixture loads from `Nibras.sln`. Each rule is one test method named as below, carries its test-case identifier as the test-case attribute, and runs in every `ci-service.yml`. The rules this document owns are TC-TST-101 to TC-TST-124 (except TC-TST-114) and TC-TST-740 to TC-TST-742; seven rows carry an identifier another document owns and cite it with its owner: TC-TST-114 (the BFF Web sheet), and TC-PLAT-971, TC-PERF-962, TC-PERF-963, TC-DATA-951, TC-DATA-959 and TC-MSG-952 (document 20, the derived acceptance tests of REQ-PLAT-021, REQ-PERF-012, REQ-PERF-013, REQ-DATA-001, REQ-DATA-009 and REQ-MSG-002). A new service is covered the moment its projects exist, because the fixture enumerates the solution rather than a list.
...
| TC-TST-740 | `GrpcHopRules.GrpcServices_MakeNoNestedCall` | No type in an `.Api` assembly that derives from a generated gRPC service base class references a generated gRPC client type, and no type in an `.Application` assembly references one either, so the only gRPC clients in a service are the `.Infrastructure` adapters TC-TST-742 lists. A call marked *job only* lives in `Jobs/`, which no gRPC service type references. This is the static half of the one-hop rule of reference architecture Section 8.0 and it cannot see the whole rule: a gRPC service that dispatches to an Application handler whose port is implemented in `.Infrastructure` by a gRPC client makes a nested call through an interface, which no reference scan follows. The runtime proof of the rule is `TC-API-072` (document 22): the outbound interceptor of document 22 §10.4 refuses a second gRPC call opened while serving one, whichever layer opens it. |
...
| `TC-PERF-962` (document 20) | `Entities_HaveNamedTenantAndSoftDeleteFilters` | A model scan over every `DbContext` in the solution finds a named `Tenant` filter and a named `SoftDelete` filter on every tenant-owned entity (REQ-PERF-012; written by SL-DATA-001 with the base context). |
| `TC-PERF-963` (document 20) | `Api_ReturnsNoDomainType_NoLazyNavigation` | No endpoint or gRPC service type in an `.Api` assembly returns or serialises a type from a `.Domain` assembly, no navigation property is `virtual`, and no project references `Microsoft.EntityFrameworkCore.Proxies` (REQ-PERF-013). |
| `TC-DATA-951` (document 20) | `Services_ConnectToOwnDatabaseOnly` | A scan of every service's connection-string configuration (`appsettings*.json`, the AppHost and the Helm values) and of its migrations finds 0 references to another service's `nibras_<service>` database or schema (REQ-DATA-001); TC-TST-113 covers the assembly half of the same requirement. |
| `TC-DATA-959` (document 20) | `Migrations_HaveNoCrossServiceForeignKey` | A scan of every service's migrations finds no foreign key to a table outside the service's own schema; a reference to another service's entity is a plain `uuid` column (REQ-DATA-009). |
| `TC-MSG-952` (document 20) | `Handlers_NeverAwaitAMessageReply` | No type in an `.Application` feature folder that maps an endpoint or a gRPC method sends a message to a RabbitMQ endpoint and awaits its reply (request-reply over a transport, which `Nibras.BuildingBlocks.Messaging` marks so the rule can find it; in-process handler calls are not affected), so a user-waiting query or command never goes through the broker (REQ-MSG-002). |

Thirty-three rules. TC-TST-120, the path rule and the three migration and configuration scans (`TC-DATA-951`, `TC-DATA-959`, and the model scan `TC-PERF-962`) are source or model scans and TC-TST-124 is a solution scan rather than a NetArchTest predicate; all live in the same project because they answer the same question, which is whether the repository has the shape this document says it has. **This table is the single list of architecture rules.** The "`Nibras.BuildingBlocks.Testing` architecture test pack" that `34-work-breakdown.md` SL-TST-003 builds is this project and these rules: the Testing block supplies the fixture that loads the solution and the shared predicates, and every rule lives here. SL-TST-003 writes TC-TST-101 to TC-TST-124 (except TC-TST-114), TC-TST-740 to TC-TST-742, `TC-PERF-963`, `TC-DATA-951`, `TC-DATA-959` and `TC-MSG-952`; SL-DATA-001 adds `TC-PERF-962` with the base context and SL-PLAT-001 the path rule. REQ-DATA-012 (split queries), which SL-TST-003 also covers, is not a structural rule: the command-counting interceptor asserts it at run time in each service's integration suite, under `TC-DATA-962` (document 20). The five rows added in this round carry document 20's identifiers because each is the derived acceptance test of its requirement, as the path rule is. `GrpcHopRules` is the class reference architecture Section 8.0 names; it holds TC-TST-740 and TC-TST-742 and nothing else. `Fixtures/SyncDependencies.cs` is the one hand-kept list the rules read: the architecture-reviewer agent compares it with reference architecture Section 8.0 row by row at every change to either, in the same pull request. The path rule is the derived acceptance test of REQ-PLAT-021, so it carries document 20's identifier rather than a new one.
...
| Claim | Proof |
|---|---|
| The repository has the shape this document gives it, and every dependency rule in 10.2 holds | Product artefact: the thirty-three rules in 10.3 are test methods in `tests/Architecture.Tests/`, each carrying its test-case identifier, run in every `ci-service.yml`. SL-TST-003 builds the project and its rule pack, SL-TST-001 puts it in the pipeline, SL-DATA-001 adds `TC-PERF-962` and SL-PLAT-001 adds the path rule. TC-TST-124 fails if `src/Services/` ever disagrees with Appendix L, TC-TST-108 fails the day a building block reaches into a domain, and TC-TST-740 fails on a gRPC client referenced from `.Api` or `.Application`. The one-hop rule itself is proved at run time by `TC-API-072` (document 22), which refuses a nested gRPC call whichever layer opens it; the static rule cannot follow a call made through an Application port |
...
| 2026-09-26 | planning session, round-3 remediation (self-check against the kit lint) | each blocking item above answered in this document; awaiting the Group B re-score | none in this document |
| 2026-09-26 | Round-5 scorecard, remediation round 6 | Amended; awaiting the round-6 score | TC-TST-740 extended to `.Application` and narrowed to what a reference scan sees, with `TC-API-072` (document 22) cited as the runtime proof of the one-hop rule here and in the verification table; 10.3 made the single list of architecture rules, adding `TC-PERF-962`, `TC-PERF-963`, `TC-DATA-951`, `TC-DATA-959` and `TC-MSG-952` (REQ-PERF-012, REQ-PERF-013, REQ-DATA-001, REQ-DATA-009, REQ-MSG-002) and stating that SL-TST-003's pack is this project, so the count is thirty-three; the note deferring the location of `GrpcHopRules` to document 22 dropped, because `22-api-conventions-and-error-catalog.md` §10.4 already places it in 10.3 |
| 2026-09-26 | Round-6 scorecard, remediation round 7 | Amended; awaiting the round-7 score | The proto paragraph says `GrpcHopRules` checks the static half of the one-hop rule and cites `TC-API-072` as its runtime proof, as the TC-TST-740 row does; part 1 quotes the team (five to eight builders) and the phase ranges of `17-roadmap.md` beside the 206-project count |
