...
| Twenty-two of the twenty-three ADRs in Section 8 (ADR-0001 to ADR-0018 and ADR-0020 to ADR-0023) are Proposed, not Accepted; only ADR-0019 is Accepted | The plan is written as though each Proposed record were accepted; `29-adr-index.md` tracks the status of each. ADR-0024, which lists the Tier 2 requirements built before Tier 1 is complete, is also Proposed and sits outside Section 8 because it decides scheduling, not architecture | Product owner, at the group reviews; the architect updates each record's status | A rejected ADR reopens its section here and every document that cites it; ADR-0001, ADR-0004 and ADR-0008 reach furthest in the architecture, and ADR-0020 to ADR-0023 in how the plan proves itself | 2 | 3 | 6 | RISK-01, RISK-44 |
...
| Every ADR in Section 8 exists with its status and the alternatives stated, and Section 8 covers ADR-0001 to ADR-0023 | `kit-lint` rule R22 fails on an ADR number cited here with no record in `docs/project/DECISIONS/`, and checks that `29-adr-index.md` indexes every record once with the status the record carries. That Section 8 lists every record from ADR-0001 to ADR-0023, marks ADR-0019 Accepted and states each record's alternatives is a review step: `architecture-reviewer` lists `docs/project/DECISIONS/` against the Section 8 rows and the open point that counts the Proposed records, and reads the "Alternatives considered" section of each, at the Group B review and whenever an ADR is added or changes status |
