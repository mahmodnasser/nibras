...
**Group** C · **Requirement areas covered** DATA, PRV (retention rows), PERF (partitioning and pooling as they touch data) · **Last updated** 2026-09-26 by the platform plan
...
| Ai | `ai_index.embedding_chunk`, chunks with tenant, permission and scope tags | Appendix E names Ai as a consumer of five source change keys, `communication.announcement.published.v1`, `assessment.report-cards.published.v1`, `academics.lesson-plan.submitted.v1`, `attendance.attendance.marked.v1` and `behavior.incident.recorded.v1`, and of `school.student.section-changed.v1`; all six bind on `ai.index-sources.bulk` (`11-messaging-architecture.md` §2.5). A change key only queues the record: its content is still read through Bff.Web's internal source feed under the Ai service credential (reference architecture Section 8.0), so no indexed text travels on the bus. `school.student.section-changed.v1` re-tags scope without re-embedding, and `school.student.status-changed.v1` on `ai.events` removes a leaver's chunks | The column list is `25-ai-and-assist-ladder.md` §4.1: `tenant_id`, `source_service`, `source_entity`, `source_id`, `source_version`, `data_class`, `required_permission`, `scope_campus_id`, `scope_section_ids`, `scope_student_ids`, `embedding` | Nightly `IndexReconciliationJob` compares each stream's count and highest `source_version` with the feed summary and queues a `reconciliation` rebuild on a mismatch; `IndexSyncJob` catches up a missed or parked change key; a rebuild is reported by `ai.index.rebuild-completed.v1` (`06-services/ai.md` §9, §10) |
...
| Read replica | `nibras_reporting` has a streaming replica in the scale mode (master brief Section 34). The Reporting API reads through a second connection string `Reporting:ReadReplica`; writes and the rebuild go to the primary. Both backends-for-frontends read Reporting's projections through the Reporting API, never through a connection string, because they own no database and may not read another service's (Appendix L, master brief Section 7.1; `06-services/bff-web.md` open point 2) |
...
| Read-your-writes | A dashboard opened immediately after an action the user just took carries the `messageId` returned by the write in an `Nibras-Expect-Message` header; the BFF forwards the header to the Reporting API, which checks the checkpoint's `last_message_id` on the replica and, if the projection has not caught up, reads the primary once. This is what stops a teacher marking attendance and then seeing yesterday's number |
...
| A reference copy from Section 6 diverges from its owner and nobody sees it, so a decision is made on a stale name or status | 3 | 3 | Rule 3 (an event older than `source_version` is discarded), rule 4 (a missing copy is a data-quality issue), and the nightly `ReferenceCopyReconciliationJob` checksum with self-repair, `TC-DATA-642` | Architect | RISK-15 |
| A hot partition at a 20,000-student tenant makes the current attendance month the bottleneck before general availability | 3 | 3 | Open point 2 keeps hash sub-partitions as a named option decided by the N-01 scale run rather than guessed now; the pruning rule and the per-partition autovacuum settings in `21-performance-engineering.md` Section 4 keep the current month planned well | Architect | none; score 9, local, and decided by the N-01 scale run through open point 2 |
| A retention job detaches or drops a partition that a legal hold covers, and the evidence is gone | 2 | 5 | The `retention_holds` check before every detach (Section 5, Appendix J rule 4), a skipped partition reported to the Data Quality Center, the checksum before the drop, and `TC-DATA-014` with `TC-PRV-902` for the hold cases | Data architect | none; score 10, local. The regulatory side of a wrong retention clock is RISK-23 |

### 12. Platform notes and signature features

| Concern | What holds here | Proof and runner |
|---|---|---|
| Runner | PostgreSQL runs only as a Linux container, so every persistence suite, `TenantIsolation.Tests` and the migration lint run on the Linux runner (`ubuntu-latest`). The tenancy and persistence building blocks (`Nibras.BuildingBlocks.*`) also run on `windows-latest`, one of the two Windows integration runs document 33 part 4 keeps (the other is `Documents`), which is where path, culture and line-ending mistakes in the building block would show. `dev-smoke` starts every database on `ubuntu-latest`, `windows-latest` and `macos-latest` | Document 33 part 4; `TC-PLAT-015` (document 33) for the pooling case on `ubuntu-latest` |
| Arabic collation and search | Every bilingual column pair carries its language's collation, `ar-x-icu` or `en-x-icu`, and the Arabic column feeds the folded search column through `nibras_ar_fold` (`24-localization-and-calendars.md`); both depend on ICU inside the PostgreSQL image, which is why the image's LC_CTYPE is UTF-8 | `TC-DATA-750` on `ubuntu-latest`; `TC-L10N-310` (document 24) for the trigram check inside the image |
| Instants and dates | Every instant is `timestamptz` stored in UTC and every calendar date is `date`, so no value depends on the host's time zone or culture; a tenant's time zone applies only when a date is shown or a job window is computed (REQ-L10N-010) | `TC-DATA-751` on `ubuntu-latest` and `windows-latest` |
| Right-to-left output | This document produces no rendered output; documents, exports and reports inherit their direction from the owners that render them (Documents, Reporting). An export keeps each column's text as stored, so an Arabic name leaves byte-identical | `TC-TST-208` (document 16) for rendered documents |
| Devices without Google services | No data rule here depends on the device's services: the device column of Section 10 applies to every Android build alike, and push is not a data path | `TC-NOT-610` (Notification sheet) for the push fallback |

**Signature features.** This document owns none. Its mechanisms are what features 2 (Student 360, from the `student_360` projection of Section 7), 9 (import with dry run and rollback, the tenant-scoped transactions it relies on), 13 (data quality center, fed by the reconciliation of Section 6), 14 (trust by design: retention jobs and single-tenant export), 27 (explain this number, through the projections' checkpoints) and 31 (guardian transparency, through the access log of Section 10) rest on, cited by their Appendix W numbers. Each feature's moment, rung, autonomy, the requirements and slices that build it, its Appendix O step and its demo test are held once, in the "Signature feature trace" table of `32-product-differentiation-and-demo.md`, and are not copied here.
...
| Collation, search folding and stored instants do not depend on the host | `TC-DATA-750` on `ubuntu-latest`; `TC-DATA-751` on `ubuntu-latest` and `windows-latest` (Section 12) | Every pull request touching persistence or a migration |
...
The brief-sources table above defines `TC-DATA-640` (a context leased without a tenant throws), `TC-DATA-641` (partitions created ahead and detached on schedule), `TC-DATA-642` (reference-copy reconciliation) and `TC-DATA-020`; `TC-DATA-001` to `TC-DATA-006` are the WF-DATA-01 import transitions of Appendix R, owned by the Documents sheet (`06-services/documents.md`) and not cited here for any other meaning. This document also defines `TC-DATA-643` (the pooled-connection test of Section 2.5), `TC-DATA-750` and `TC-DATA-751` (the platform tests of Section 12) and:
...
| TC-DATA-750 | Given the Nibras PostgreSQL image on `ubuntu-latest` and every service model, when the persistence convention test reads every `<field>_ar` and `<field>_en` column, then each carries `ar-x-icu` or `en-x-icu`, and twenty Arabic fixture names with and without hamza, ta marbuta and diacritics sort in the expected order under `ORDER BY name_ar` and are all found by a folded search for their unmarked form | REQ-L10N-009, REQ-PLAT-015 |
| TC-DATA-751 | Given the persistence building block's suite run once with the host time zone `UTC` and once with `Asia/Amman`, and with the process culture `ar-SA`, on `ubuntu-latest` and `windows-latest`, when an entity with an instant and a calendar date is saved and reloaded, then the instant round-trips unchanged in UTC, the date does not move by a day, and no stored value is formatted in the Um Al Qura calendar | REQ-L10N-010, REQ-PLAT-019 |
