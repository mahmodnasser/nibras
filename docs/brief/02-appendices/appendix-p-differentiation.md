...
|---|---|---|---|
| **openSIS** | Open source, self-hosted | Free, straightforward student information, long history | Dated interface, limited mobile, little workflow, no real multi-tenancy |
| **Gibbon** | Open source, self-hosted | Broad module coverage, active community | Interface and mobile, performance at size, limited finance |
| **Fedena** | Commercial, hosted or on-premises | Broad modules, strong in South Asia and the Gulf, competitive price | Depth per module, configurability without code, modern interface |
| **PowerSchool** | Commercial, enterprise | Very deep student information, compliance reporting, ecosystem | Cost, complexity, slow to configure, weaker outside North America |
| **Classter** | Commercial, hosted | Broad, configurable, good integrations | Interface density, cost at small scale |
| **Veracross** | Commercial, hosted | Independent schools, one database, strong advancement | Price, fit outside independent schools |
| **ManageBac** | Commercial, hosted | International Baccalaureate curriculum depth, parent experience | Narrow outside its curricula, limited finance and human resources |
| **Blackbaud** | Commercial, enterprise | Fundraising and finance depth | Cost, complexity, integration-heavy |
| **Toddle** | Commercial, hosted | Teaching and learning experience, curriculum planning | Not a full school management system |
| **Classera** | Commercial, hosted | Arabic-first, regional presence, ministry alignment | Depth in finance and operations, configurability |
...
|---|---|---|
| **Arabic and English are equal, not translated.** Right-to-left layouts, bilingual names and documents, Arabic-aware search, Hijri display, amounts in words in both languages | Schools in the Gulf and the Levant | Four-way visual snapshots, bilingual PDF baselines, the Arabic normalization rules in Appendix S |
...
| **Live in a day.** Smart defaults from country and school type, import with dry run and rollback | The decision maker's biggest fear | TC-PLT-003, TC-DOC-001, the 10,000-row import scenario in Appendix N |
| **Open by default.** Public API, webhooks, iCal, OneRoster, data export always available, read-only before suspension | Schools that have been locked in before | TC-INT-001, and the dunning policy in master brief Section 36 |
| **Open source, honestly.** No licence fee, two named non-open exceptions, every dependency verified | Owners and ministries | The dependency inventory and the licence scan |
...
| An app and integration marketplace | PowerSchool, Classter | Tier 2. The plug-in kit and template exchange are the first two steps |
| Cafeteria and prepaid wallet | Fedena, Classter | Tier 3, in Operations, with the parent wallet as the Tier 2 precursor |
| Alumni and community management | Veracross, Blackbaud | Tier 3, as a student status plus a profile in School |
...
`/differentiate` fills one row at a time and writes it here. The columns are fixed so the finished matrix can be read as a sales sheet and as a review artefact.
...
|---|---|---|---|---|---|
| 26 Exception-only attendance | Partial: several have gate integration; few pre-fill the register from it | Three sources merged into the register before the teacher opens it, with the conflict rules to make it safe | "The register was already half filled" | Minute 2, TC-ATT-003 | Low. The integration depth is the moat, not the idea |
...
| 29 Smart defaults engine | Rare. Most have a setup wizard with empty fields | Country plus school type infers eleven configuration areas and shows what it inferred | "It already knew how our year works" | Minute 15, TC-PLT-003 | Medium |
| 31 Guardian transparency | Very rare | A parent sees which roles read their child's sensitive records | "I can see who looked at my child's file" | Minute 11, TC-AUD-002 | Low. It requires an access log most products do not keep |
| 32 Emergency mode with reunification | Some, usually as a separate paid product | Built in, offline-capable, with acknowledgement tracking | "Every child accounted for in four minutes" | Minute 13, TC-ATT-004 | Medium |
| 44 Low-bandwidth mode | Rare | A data-saver profile designed for the phones parents actually own | "It worked on my old phone" | Not in the fifteen-minute script; TC-MOB-005 | Low |
