...
p('|---|---|---|---|---|---|---|');
p('| The ranges assume five to eight builders from the first week of phase 1 | Recomputed with `schedule-34.mjs` whenever the team differs, and at the end of every phase from the slices actually delivered | Architect and product owner | 4 | 3 | 12 | RISK-03 |');
...
p('| SL-FIN-444 needs a country e-invoicing plug-in in phase 3, and Open Question 9, still open for e-invoicing, decides which plug-in is built there | The e-invoicing plug-in for the country of a VAT-registered first customer is built in phase 3 by the Finance owner\'s team, other country implementations in phase 5, as the conditional slices SL-FIN-448 to SL-FIN-450 (ZATCA) and SL-FIN-451 and SL-FIN-452 (JoFotara), counted in phase 3 | Product owner, at the Phase 2 exit review | 4 | 3 | 12 | RISK-53 |');
p();
p('## Review record');
p();
...
p('| 2026-09-26 | Round-5 scorecard, remediation round 6 | Approved with minor gaps. Amended: LTI Assignment and Grade Services built by SL-INT-412 (Platform) and SL-ASM-400 (Assessment) under CAP-INT-02, 3 slice-days in phase 4 with no change to the phase 4 range of document 17, and SL-ACA-405 no longer promises a grade on its own; the JoFotara certification of SL-FIN-452 names `TC-INT-037` and the ZATCA certification of SL-FIN-450 `TC-FIN-620`; the ZATCA estimate carries a low-confidence note under RISK-53 |');
p('| 2026-09-26 | Round-6 scorecard, remediation round 7 | Group F approved with minor gaps. Amended: SL-FIN-444 and the SL-FIN-444 risk row state Open Question 9\'s default as `OPEN_QUESTIONS.md` does (the conditional phase 3 plug-in for a VAT-registered first customer\'s country); the part C coverage note gives the phase 3 sensitivity at the ZATCA upper bound (12 to 18 weeks, 62 to 95 to launch) and says `RecordToolScore` is listed in document 11 section 2.4 |');
p('| 2026-09-26 | Open Question 30 decided (ADR-0027) | Amended: SL-PLT-010 states the decision, the master brief Section 36 count computed by Platform as BR-FIN-017 (ADR-0027, Accepted, brief v9.7), instead of the conflict; the open decision and the open point on the billing count are removed, because RISK-52 is Closed |');
